        ...
        "screens": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "screen_type": {"type": "string", "enum": ["events", "summary", "other_stats", "unknown"]},
                    "confidence": {"type": "string", "enum": ["high", "medium", "low"]},
                },
                "required": ["image_index", "screen_type", "confidence"],
            },
        },
        "summary": {
            "type": "object",
            ...
            "properties": {
                ...
                "source_image_indices": {"type": "array", "items": {"type": "integer", "minimum": 1}},
                "left_label": {"type": ["string", "null"]},
                "right_label": {"type": ["string", "null"]},
                "left_participant_slot": {"type": "string", "enum": ["home", "away", "unknown"]},
                "right_participant_slot": {"type": "string", "enum": ["home", "away", "unknown"]},
                "mapping_confidence": {"type": "string", "enum": ["high", "medium", "low"]},
                "summary_confidence": {"type": "string", "enum": ["high", "medium", "low"]},
                "expected_goals_left": {"type": ["number", "null"]},
                ...
                "pass_accuracy_right": {"type": ["number", "null"]},
                "tackles_left": {"type": ["integer", "null"], "minimum": 0},
                "tackles_right": {"type": ["integer", "null"], "minimum": 0},
                "tackles_won_left": {"type": ["integer", "null"], "minimum": 0},
                "tackles_won_right": {"type": ["integer", "null"], "minimum": 0},
                "interceptions_left": {"type": ["integer", "null"], "minimum": 0},
                "interceptions_right": {"type": ["integer", "null"], "minimum": 0},
                "saves_left": {"type": ["integer", "null"], "minimum": 0},
                "saves_right": {"type": ["integer", "null"], "minimum": 0},
                "fouls_left": {"type": ["integer", "null"], "minimum": 0},
                "fouls_right": {"type": ["integer", "null"], "minimum": 0},
                "offsides_left": {"type": ["integer", "null"], "minimum": 0},
                "offsides_right": {"type": ["integer", "null"], "minimum": 0},
                "corners_left": {"type": ["integer", "null"], "minimum": 0},
                "corners_right": {"type": ["integer", "null"], "minimum": 0},
                "raw_metrics": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        ...
                        "properties": {
                            "label": {"type": "string"},
                            "left_value": {"type": ["string", "null"]},
                            "right_value": {"type": ["string", "null"]},
                        },
                        "required": ["label", "left_value", "right_value"],
                    },
                },
                "notes": {"type": "array", "items": {"type": "string"}},
            },
            "required": [
                ...
                "fouls_left", "fouls_right", "offsides_left", "offsides_right", "corners_left", "corners_right",
                "raw_metrics", "notes"
            ],
        },
    },
...
def _summary_classifier_prompt(context: dict[str, Any]) -> str:
    ...
    return f"""You classify EA Sports FC post-match screenshots and extract the Summary tab when present.
All images belong to ONE FIFA Night match. The game version from the app is {context.get('game_version','unknown')}.

AUTHORITATIVE FIFA NIGHT CONTEXT:
- HOME bracket slot: player {home.get('player_name','')}; assigned club/team {home.get('team','')}.
- AWAY bracket slot: player {away.get('player_name','')}; assigned club/team {away.get('team','')}.

For every image return one classification:
- events = the Events timeline tab is selected / visible
- summary = the Summary tab is selected / visible
- other_stats = another post-match tab such as Possession, Shooting, Passing or Defending is selected
...
def _summary_to_storage(summary: dict[str, Any], context: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    left_slot=str(summary.get("left_participant_slot") or "unknown")
    ...
    if mapping_ok:
        for field in fields:
            lv=summary.get(f"{field}_left");rv=summary.get(f"{field}_right")
            if left_slot=="home": home_stats[field]=lv;away_stats[field]=rv
            else: home_stats[field]=rv;away_stats[field]=lv
    else:
        home_stats={field:None for field in fields};away_stats={field:None for field in fields}
    stats={
        "home":home_stats,"away":away_stats,
        "usable_for_awards":bool(mapping_ok),
        "source_image_indices":sorted({int(x) for x in (summary.get("source_image_indices") or []) if str(x).isdigit()}),
        "raw":{
            "left_label":summary.get("left_label"),"right_label":summary.get("right_label"),
            "left_participant_slot":left_slot,"right_participant_slot":right_slot,
            "raw_metrics":summary.get("raw_metrics") or [],"notes":summary.get("notes") or [],
        },
    }
    confidence={
        "summary":summary.get("summary_confidence") or "low",
        "mapping":summary.get("mapping_confidence") or "low",
        "mapping_ok":mapping_ok,
    }
    return stats,confidence


def _empty_event_scan_result(context: dict[str, Any]) -> dict[str, Any]:
    blank={
        "left_label":None,"right_label":None,"score_left":None,"score_right":None,"match_clock":None,
        "score_confidence":"low","events":[],"event_list_complete":False,"needs_more_images":True,"notes":["Brak zdjęcia zakładki Events w tej paczce."],
        "left_participant_slot":"unknown","right_participant_slot":"unknown","mapping_confidence":"low",
        "shootout_left":None,"shootout_right":None,
    }
    return _map_scan_to_fifa_context(blank,context)
...
async def _openai_structured_vision(
    ...
    *,
    ...
    body={
        ...
        "text":{"verbosity":"low","format":{"type":"json_schema","name":schema_name,"strict":True,"schema":schema}},
    }
...
async def match_scan_preview(
    ...
    """Mixed Events + Summary scan for one real FIFA Night match.

    Event rows remain preview-only until the existing result confirmation flow saves
    them. A parsed Summary snapshot is upserted immediately but stays inactive for
    Awards until the match has a real, non-forfeit result.
    """
    ...
    for index,image in enumerate(images,start=1):
        ...
        normalized,normalized_type,width,height=_normalize_image_for_google(raw,index)
        prepared.append({
            "image_index":index,"filename":image.filename or f"image_{index}","content_type":content_type,"bytes":len(raw),
            "normalized":normalized,"normalized_content_type":normalized_type,"normalized_bytes":len(normalized),"width":width,"height":height,
        })

    total_started=time.perf_counter()
    summary_saved=False
    summary_meta: dict[str, Any]={"detected":False,"saved":False,"usable_for_awards":False,"parser_version":SUMMARY_PARSER_VERSION}
    classifier_usage: dict[str, Any]={}
    classifier_elapsed=0
    event_prepared=prepared

    # First pass: classify every screenshot and extract Summary only. If this
    # auxiliary pass ever fails, keep the old Events scanner working by falling
    # back to all images instead of breaking result entry.
    try:
        classified,classifier_payload,classifier_elapsed=await _openai_structured_vision(
            prepared,_summary_classifier_prompt(context),AI_SCREEN_CLASSIFY_SUMMARY_SCHEMA,"ea_fc_screen_classifier_summary",timeout_seconds=60.0
        )
        classifier_usage=classifier_payload.get("usage") or {}
        screen_types={}
        for row in classified.get("screens") or []:
            if not isinstance(row,dict):continue
            try:idx=int(row.get("image_index"))
            except Exception:continue
            screen_types[idx]=str(row.get("screen_type") or "unknown")
        # Unknown remains in the Events pass as a conservative fallback. Confirmed
        # Summary/other-stat tabs are kept away from the Events timeline parser.
        event_prepared=[x for x in prepared if screen_types.get(int(x["image_index"]),"unknown") in {"events","unknown"}]
        summary=classified.get("summary") or {}
        if bool(summary.get("present")):
            summary_meta["detected"]=True
            stats,confidence=_summary_to_storage(summary,context)
            db.upsert_match_summary_stats(
                tournament_id,int(match_no),str(context.get("game_version") or "FC26"),SUMMARY_PARSER_VERSION,stats,confidence
            )
            summary_saved=True
            summary_meta.update({
                "saved":True,"usable_for_awards":bool(stats.get("usable_for_awards")),
                "source_image_indices":stats.get("source_image_indices") or [],
                "mapping_confidence":confidence.get("mapping"),"summary_confidence":confidence.get("summary"),
            })
    except HTTPException as exc:
        summary_meta["warning"]="Klasyfikacja Summary nie powiodła się; Events zeskanowano w trybie zgodności."
        summary_meta["classifier_status_code"]=exc.status_code
        event_prepared=prepared

    event_payload: dict[str, Any]={}
    event_elapsed=0
    if event_prepared:
        event_result,event_payload,event_elapsed=await _openai_structured_vision(
            event_prepared,_match_event_scan_prompt(context),MATCH_EVENT_SCAN_SCHEMA,"ea_fc_match_scan",timeout_seconds=75.0
        )
        event_result=_map_scan_to_fifa_context(event_result,context)
        event_result=_resolve_scan_footballer_names(event_result,context)
    else:
        event_result=_empty_event_scan_result(context)

    event_usage=event_payload.get("usage") or {}
    input_tokens=int(classifier_usage.get("input_tokens") or 0)+int(event_usage.get("input_tokens") or 0)
    output_tokens=int(classifier_usage.get("output_tokens") or 0)+int(event_usage.get("output_tokens") or 0)
    total_tokens=int(classifier_usage.get("total_tokens") or 0)+int(event_usage.get("total_tokens") or 0)
    if total_tokens<=0: total_tokens=input_tokens+output_tokens
    estimated_cost=input_tokens*0.20/1_000_000+output_tokens*1.20/1_000_000
    elapsed_ms=round((time.perf_counter()-total_started)*1000)
    return {
        "provider":"openai_luna","model":"gpt-5.6-luna","processing_time_ms":elapsed_ms,"image_count":len(prepared),
        "context":context,"result":event_result,
        "usage":{"input_tokens":input_tokens,"output_tokens":output_tokens,"total_tokens":total_tokens},
        "estimated_cost_usd":round(estimated_cost,8),
        "saved_to_database":False,
        "summary_scan":summary_meta,
        "summary_saved_to_database":summary_saved,
        "phase_times_ms":{"classify_summary":classifier_elapsed,"events":event_elapsed},
    }
...
@app.get("/api/v1/gala/{year}")
def get_awards_gala(year: int) -> dict[str, Any]:
    if year < 2020 or year > 2100: raise HTTPException(422, "Nieprawidłowy rok.")
    return db.awards_gala_status(int(year))


@app.post("/api/v1/gala/{year}/start")
def start_awards_gala(year: int, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    _require_controller_header(authorization)
    try:return db.start_awards_gala(int(year))
    except ValueError as exc:raise HTTPException(409,str(exc)) from exc


@app.post("/api/v1/gala/{year}/next")
def next_awards_gala(year: int, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    _require_controller_header(authorization)
    try:return db.advance_awards_gala(int(year))
    except ValueError as exc:raise HTTPException(409,str(exc)) from exc


@app.post("/api/v1/gala/{year}/replay")
def replay_awards_gala(year: int, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    _require_controller_header(authorization)
    try:return db.replay_awards_gala_category(int(year))
    except ValueError as exc:raise HTTPException(409,str(exc)) from exc
...
def reset_awards_gala(year: int, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    _require_controller_header(authorization)
    return db.reset_awards_gala(int(year))
