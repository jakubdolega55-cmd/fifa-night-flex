...
function renderIntro(s,c){return `${topLine(s)}<div class="stage intro"><div class="eyebrow">KATEGORIA ${Number(s.current_index)+1} / ${s.total_categories}</div><h1 class="category">${esc(c.title||'')}</h1><div class="quote">${esc(c.opening_text||'')}</div>${c.second_opening_text?`<div class="quote second">${esc(c.second_opening_text)}</div>`:''}</div>`}
