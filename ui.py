...
def render_synced_setup_tv(tid: str, api_url: str):
    ...
    (()=>{{
      ...
      function play(ev){{playing=true; const p=ev.payload||{{}};
        ...
        if(ev.kind==='special_draw'){{headline.textContent='🎲 Losowanie w trakcie turnieju';const pairs=(p.pairs||[]).map((x,i)=>`<div class="pair" style="--d:${{.9+i*4.5}}s"><small>M${{esc(x.match_no||'')}}${{x.stage?' • '+esc(x.stage):''}}</small><b>${{esc(x.home_name||'?')}}</b><span>VS</span><b>${{esc(x.away_name||'?')}}</b></div>`).join('');let lucky=p.selected_lucky?.name||p.name||'';root.innerHTML=`<div style="width:100%"><div class="specialTitle">${{p.special_kind==='group_playoffs'?'Faza pucharowa':p.special_kind==='double7_combined'?'Winners + Szczęśliwy los':'Losowanie drabinki'}}</div><div class="specialGrid">${{pairs}}</div>${{lucky?`<div class="lucky slowLucky" style="animation-delay:${{.9+(p.pairs||[]).length*4.5}}s">🍀 ${{esc(lucky)}}</div>`:''}}</div>`;finish(3000+Math.max((p.pairs||[]).length-1,0)*4500+(lucky?4500:0));return}}
