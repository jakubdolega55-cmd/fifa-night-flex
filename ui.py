...
def render_awards_gala_tv(initial_status: dict, api_url: str, year: int):
    """Fullscreen-style FIFA Night Awards TV surface.

    The iframe polls the public Gala GET endpoint itself, so phone-triggered state
    changes appear smoothly without forcing a Streamlit rerun every second.
    """
    import json
...
<style>
html,body{margin:0;background:transparent;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#f8fafc}
*{box-sizing:border-box}.gala-shell{--gold:#f7c948;--gold2:#fff0a8;position:relative;min-height:760px;overflow:hidden;border-radius:30px;border:1px solid rgba(255,255,255,.10);background:radial-gradient(circle at 50% -8%,rgba(247,201,72,.20),transparent 34%),radial-gradient(circle at 8% 82%,rgba(37,99,235,.18),transparent 32%),linear-gradient(145deg,#050914 0%,#0b1220 47%,#111827 100%);box-shadow:0 28px 90px rgba(0,0,0,.38);padding:44px 54px}
.gala-shell:before{content:"";position:absolute;inset:-2px;background:linear-gradient(115deg,transparent 28%,rgba(255,255,255,.035) 48%,transparent 68%);transform:translateX(-110%);animation:sheen 8s ease-in-out infinite;pointer-events:none}@keyframes sheen{0%,55%{transform:translateX(-110%)}85%,100%{transform:translateX(110%)}}
.topline{position:relative;z-index:2;display:flex;align-items:center;justify-content:space-between;gap:20px;color:#cbd5e1;text-transform:uppercase;letter-spacing:.19em;font-weight:900;font-size:12px}.brand{color:var(--gold2)}.step{padding:8px 13px;border-radius:999px;border:1px solid rgba(247,201,72,.22);background:rgba(247,201,72,.07)}
.stage{position:relative;z-index:2;min-height:650px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}.eyebrow{font-size:13px;letter-spacing:.23em;text-transform:uppercase;color:#f7c948;font-weight:950;margin-bottom:16px}.category{font-size:clamp(34px,5vw,68px);font-weight:1000;line-height:1.02;letter-spacing:-.045em;margin:0;max-width:1180px;text-wrap:balance}.quote{max-width:1050px;font-size:clamp(22px,3vw,39px);line-height:1.24;color:#e2e8f0;font-weight:720;margin:28px auto 0;text-wrap:balance}.quote.second{font-size:clamp(18px,2.35vw,31px);color:#fde68a;opacity:0;animation:fadeUp .7s ease 2.6s forwards}@keyframes fadeUp{to{opacity:1;transform:none}}.intro .category{animation:introTitle .75s cubic-bezier(.2,.8,.2,1) both}.intro .quote{animation:introQuote .75s ease .35s both}@keyframes introTitle{from{opacity:0;transform:translateY(18px) scale(.98)}to{opacity:1;transform:none}}@keyframes introQuote{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}
.cinematic-intro{position:relative;isolation:isolate}.cinematic-layer{position:absolute;inset:76px 18px 28px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;opacity:0;pointer-events:none}.cinematic-fixed{position:absolute;top:92px;left:0;right:0;z-index:3}.cinematic-question{font-size:clamp(54px,8.2vw,118px);font-weight:1000;letter-spacing:-.055em;line-height:.98;text-wrap:balance}.cinematic-bigline{font-size:clamp(42px,6.2vw,88px);font-weight:1000;line-height:1.02;letter-spacing:-.045em}.accent-soft{color:#fde68a;margin-top:18px}.cinematic-final-lines{font-size:clamp(32px,5vw,66px);font-weight:950;line-height:1.18;letter-spacing:-.035em}.cinematic-final-lines strong{display:block;color:var(--gold);font-size:1.12em;margin-top:12px}.cinematic-quote{font-size:clamp(32px,4.9vw,66px)!important;max-width:1180px!important}.player-year-cinematic:after,.match-year-cinematic:after,.late-cinematic:after,.progress-cinematic:after{content:"";position:absolute;inset:0;background:#030712;opacity:0;pointer-events:none;z-index:-1;animation:cinematicDim 9s linear both}@keyframes cinematicDim{0%,30%{opacity:0}39%,43%{opacity:.62}50%,61%{opacity:.16}68%,72%{opacity:.58}80%,100%{opacity:.06}}
.py-beat-1{animation:pyBeat1 9s linear both}.py-beat-2{animation:pyBeat2 9s linear both}.py-beat-3{animation:pyBeat3 9s linear both}@keyframes pyBeat1{0%{opacity:0;transform:translateY(14px)}6%,32%{opacity:1;transform:none}39%,100%{opacity:0;transform:scale(.985)}}@keyframes pyBeat2{0%,39%{opacity:0;transform:scale(.9);filter:blur(7px)}46%,61%{opacity:1;transform:none;filter:none}68%,100%{opacity:0;transform:scale(1.04)}}@keyframes pyBeat3{0%,68%{opacity:0;transform:translateY(18px)}75%,100%{opacity:1;transform:none}}
.match-beat-1{animation:matchBeat1 9s linear both}.match-beat-2{animation:matchBeat2 9s linear both}.match-beat-3{animation:matchBeat3 9s linear both}@keyframes matchBeat1{0%{opacity:0;transform:translateY(14px)}5%,22%{opacity:1;transform:none}28%,100%{opacity:0}}@keyframes matchBeat2{0%,23%{opacity:0;transform:scale(.98)}30%,55%{opacity:1;transform:none}62%,100%{opacity:0;filter:blur(4px)}}@keyframes matchBeat3{0%,60%{opacity:0;transform:scale(.86);filter:blur(8px)}69%,100%{opacity:1;transform:none;filter:none}}
.cinematic-clock{font-size:clamp(92px,15vw,220px);font-weight:1000;letter-spacing:-.075em;font-variant-numeric:tabular-nums;color:#f8fafc;text-shadow:0 0 34px rgba(255,255,255,.08)}.cinematic-clock.danger{color:var(--gold);text-shadow:0 0 46px rgba(247,201,72,.26)}.late-beat-1{animation:lateBeat1 9s linear both}.late-beat-2{animation:lateBeat2 9s linear both}.late-beat-3{animation:lateBeat3 9s linear both}.late-beat-4{animation:lateBeat4 9s linear both}@keyframes lateBeat1{0%{opacity:0;transform:scale(.92)}4%,16%{opacity:1;transform:none}21%,100%{opacity:0;transform:scale(1.04)}}@keyframes lateBeat2{0%,18%{opacity:0;transform:scale(.92)}23%,34%{opacity:1;transform:none}39%,100%{opacity:0;transform:scale(1.04)}}@keyframes lateBeat3{0%,36%{opacity:0;transform:scale(.9)}42%,54%{opacity:1;transform:scale(1.05)}59%,100%{opacity:0;transform:scale(1.12)}}@keyframes lateBeat4{0%,57%{opacity:0;transform:translateY(16px)}65%,100%{opacity:1;transform:none}}
.progress-label{font-size:clamp(28px,4vw,54px);font-weight:1000;letter-spacing:.08em;color:#cbd5e1}.progress-meter{width:min(760px,82vw);height:34px;border-radius:999px;margin-top:28px;background:rgba(148,163,184,.14);border:1px solid rgba(148,163,184,.18);padding:5px;overflow:hidden}.progress-meter span{display:block;height:100%;border-radius:999px;background:linear-gradient(90deg,#64748b,#94a3b8)}.progress-meter .low{width:28%}.progress-meter .high{width:86%;background:linear-gradient(90deg,#d6a928,#fde68a);box-shadow:0 0 26px rgba(247,201,72,.28)}.progress-beat-1{animation:progressBeat1 9s linear both}.progress-beat-2{animation:progressBeat2 9s linear both}.progress-beat-3{animation:progressBeat3 9s linear both}@keyframes progressBeat1{0%{opacity:0;transform:translateX(-28px)}5%,31%{opacity:1;transform:none}38%,100%{opacity:0;transform:translateX(-18px)}}@keyframes progressBeat2{0%,34%{opacity:0;transform:translateX(28px)}41%,65%{opacity:1;transform:none}72%,100%{opacity:0;transform:translateX(18px)}}@keyframes progressBeat3{0%,68%{opacity:0;transform:translateY(16px)}76%,100%{opacity:1;transform:none}}
.nominees-title{font-size:clamp(30px,4vw,52px);font-weight:1000;margin:6px 0 26px}.nominee-list{width:min(1120px,96%);display:grid;gap:14px}.nominee{display:grid;grid-template-columns:minmax(280px,.9fr) minmax(320px,1.1fr);gap:24px;align-items:center;text-align:left;padding:20px 26px;border-radius:21px;background:linear-gradient(100deg,rgba(15,23,42,.92),rgba(30,41,59,.72));border:1px solid rgba(148,163,184,.22);box-shadow:0 14px 34px rgba(0,0,0,.18);animation:nomineeIn .55s cubic-bezier(.18,.84,.2,1) both}@keyframes nomineeIn{from{opacity:0;transform:translateY(16px) scale(.985)}to{opacity:1;transform:none}}.nominee-name{font-size:clamp(23px,2.7vw,36px);font-weight:1000;letter-spacing:-.025em;overflow-wrap:anywhere}.nominee-stats{display:flex;flex-wrap:wrap;gap:8px 14px;color:#cbd5e1;font-size:clamp(14px,1.5vw,19px);font-weight:750}.stat-chip{padding:8px 11px;border-radius:12px;background:rgba(148,163,184,.08);border:1px solid rgba(148,163,184,.13)}
.suspense .award-to{font-size:clamp(18px,2vw,27px);letter-spacing:.25em;color:#cbd5e1;font-weight:950;text-transform:uppercase}.suspense .dots{font-size:clamp(52px,9vw,130px);font-weight:1000;color:var(--gold);letter-spacing:.08em;animation:pulse 1.05s ease-in-out infinite}@keyframes pulse{50%{opacity:.42;transform:scale(.985)}}.suspense .line{width:min(520px,70vw);height:2px;background:linear-gradient(90deg,transparent,var(--gold),transparent);margin:30px auto;animation:lineGrow 1.4s ease-in-out infinite alternate}@keyframes lineGrow{from{transform:scaleX(.35);opacity:.35}to{transform:scaleX(1);opacity:1}}
.winner-card{width:min(1180px,97%);padding:34px 42px;border-radius:28px;background:radial-gradient(circle at 50% 0,rgba(247,201,72,.15),transparent 42%),linear-gradient(145deg,rgba(40,31,8,.72),rgba(11,18,32,.92) 46%);border:1px solid rgba(247,201,72,.42);box-shadow:0 25px 80px rgba(0,0,0,.38)}.winner-anim .winner-card{animation:winnerIn 1.05s cubic-bezier(.12,.88,.18,1) both}@keyframes winnerIn{from{opacity:0;transform:scale(.88);filter:blur(8px)}to{opacity:1;transform:none;filter:none}}.winner-label{font-size:13px;letter-spacing:.23em;text-transform:uppercase;color:#fde68a;font-weight:950}.winner-name{font-size:clamp(46px,7vw,92px);font-weight:1000;line-height:.98;letter-spacing:-.055em;margin:12px 0 20px;text-wrap:balance;overflow-wrap:anywhere}.winner-lines{display:flex;flex-wrap:wrap;justify-content:center;gap:10px 12px;max-width:1000px;margin:0 auto}.winner-line{font-size:clamp(15px,1.65vw,21px);font-weight:800;color:#e2e8f0;padding:10px 14px;border-radius:14px;background:rgba(255,255,255,.055);border:1px solid rgba(255,255,255,.085)}.hero-metric{font-size:clamp(31px,4.7vw,62px);font-weight:1000;color:#f7c948;margin:10px 0 22px;letter-spacing:-.035em}.hero-metric.anim{animation:heroPop 1.35s cubic-bezier(.12,.88,.18,1) .55s both}@keyframes heroPop{0%{opacity:0;transform:scale(.78)}70%{opacity:1;transform:scale(1.07)}100%{opacity:1;transform:scale(1)}}.hold-note{margin-top:24px;color:#64748b;font-size:11px;letter-spacing:.17em;text-transform:uppercase}
.goal-counter{font-size:clamp(54px,8vw,110px);font-weight:1000;color:#f7c948;letter-spacing:-.06em;margin:8px 0 18px}.equation{display:flex;justify-content:center;align-items:center;gap:15px;flex-wrap:wrap;font-size:clamp(22px,3.1vw,42px);font-weight:1000;margin:12px 0 24px}.equation.anim span{opacity:0;animation:eqIn .38s ease forwards}.equation.anim span:nth-child(1){animation-delay:.35s}.equation.anim span:nth-child(2){animation-delay:.75s}.equation.anim span:nth-child(3){animation-delay:1.05s}.equation.anim span:nth-child(4){animation-delay:1.45s}.equation.anim span:nth-child(5){animation-delay:1.8s}@keyframes eqIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}}.equation .result{color:#86efac;font-size:1.25em}.vs-grid{display:grid;grid-template-columns:1fr auto 1fr;gap:26px;align-items:center;width:100%;margin:14px 0 22px}.vs-side{position:relative;padding:22px 18px;border-radius:22px;min-height:150px;display:flex;flex-direction:column;align-items:center;justify-content:center;background:rgba(255,255,255,.025);border:1px solid rgba(255,255,255,.06)}.vs-name{font-size:clamp(31px,4.5vw,61px);font-weight:1000;overflow-wrap:anywhere}.vs-mark{font-size:clamp(22px,2.5vw,34px);font-weight:1000;color:#64748b}.medal{display:inline-block;margin-top:9px;padding:7px 13px;border-radius:999px;font-size:12px;letter-spacing:.12em;font-weight:950}.gold{background:#4a3508;color:#fde68a;border:1px solid #a97c14}.transparent{background:rgba(226,232,240,.07);color:#e2e8f0;border:1px solid rgba(226,232,240,.32)}
.special-vs-card{overflow:hidden;position:relative}.special-vs-card:before,.special-vs-card:after{content:"";position:absolute;top:-20%;bottom:-20%;width:38%;filter:blur(34px);opacity:.14;pointer-events:none}.special-vs-card:before{left:-12%;background:radial-gradient(circle,#94a3b8,transparent 66%)}.special-vs-card:after{right:-12%;background:radial-gradient(circle,#f7c948,transparent 66%)}.special-vs-card .winner-label{margin-bottom:12px}.winner-anim .special-vs-card .vs-side.left{animation:vsLeftIn .85s cubic-bezier(.16,.86,.2,1) .18s both}.winner-anim .special-vs-card .vs-side.right{animation:vsRightIn .85s cubic-bezier(.16,.86,.2,1) .18s both}.winner-anim .special-vs-card .vs-mark{animation:vsMarkIn .6s cubic-bezier(.12,.9,.2,1) 1.0s both}.winner-anim .special-vs-card .medal{animation:medalPop .55s cubic-bezier(.12,.9,.2,1) 1.65s both}.winner-anim .special-vs-card .vs-side.is-winner{animation-name:vsWinnerIn}.winner-hold .special-vs-card .vs-side.is-winner{box-shadow:0 0 0 1px rgba(247,201,72,.24),0 0 38px rgba(247,201,72,.13);border-color:rgba(247,201,72,.38);background:radial-gradient(circle at 50% 0,rgba(247,201,72,.12),rgba(255,255,255,.025) 58%)}
@keyframes vsLeftIn{from{opacity:0;transform:translateX(-90px) scale(.94)}to{opacity:1;transform:none}}@keyframes vsRightIn{from{opacity:0;transform:translateX(90px) scale(.94)}to{opacity:1;transform:none}}@keyframes vsWinnerIn{0%{opacity:0;transform:translateY(28px) scale(.88)}70%{opacity:1;transform:translateY(0) scale(1.055);box-shadow:0 0 0 2px rgba(247,201,72,.38),0 0 54px rgba(247,201,72,.23)}100%{opacity:1;transform:none;box-shadow:0 0 0 1px rgba(247,201,72,.24),0 0 38px rgba(247,201,72,.13)}}@keyframes vsMarkIn{from{opacity:0;transform:scale(2.3) rotate(-8deg);filter:blur(5px)}to{opacity:1;transform:none;filter:none}}@keyframes medalPop{from{opacity:0;transform:translateY(8px) scale(.65)}to{opacity:1;transform:none}}
.match-score{font-size:clamp(48px,7vw,96px);font-weight:1000;color:#f8fafc;letter-spacing:-.055em;line-height:1;margin:2px 0 8px;font-variant-numeric:tabular-nums}.match-pen{font-size:clamp(14px,1.6vw,20px);font-weight:900;color:#fde68a;letter-spacing:.08em;text-transform:uppercase}.winner-anim .match-score{animation:scorePop .72s cubic-bezier(.12,.9,.2,1) 1.08s both}.winner-anim .match-pen{animation:fadeUp .5s ease 1.65s both;opacity:0}@keyframes scorePop{from{opacity:0;transform:scale(1.7);filter:blur(6px)}to{opacity:1;transform:none;filter:none}}
.rivalry-record{font-size:clamp(34px,5.2vw,74px);font-weight:1000;color:#fde68a;letter-spacing:-.04em;line-height:1;margin:2px 0 8px}.winner-anim .rivalry-record{animation:scorePop .72s cubic-bezier(.12,.9,.2,1) 1.08s both}.clock{font-size:clamp(58px,9vw,128px);font-variant-numeric:tabular-nums;font-weight:1000;color:#f7c948;letter-spacing:-.07em;margin:8px 0 20px}.before-after{display:grid;grid-template-columns:1fr auto 1fr;gap:18px;align-items:center;margin:12px auto 24px;max-width:980px}.ba-box{padding:18px 24px;border-radius:18px;background:rgba(255,255,255,.055);border:1px solid rgba(255,255,255,.09)}.before-after.anim .ba-box,.before-after.anim .ba-arrow{opacity:0;animation:baIn .55s ease forwards}.before-after.anim .ba-box:first-child{animation-delay:.35s}.before-after.anim .ba-arrow{animation-delay:1.0s}.before-after.anim .ba-box:last-child{animation-delay:1.45s}@keyframes baIn{from{opacity:0;transform:translateY(12px) scale(.97)}to{opacity:1;transform:none}}.ba-box span{display:block;color:#94a3b8;font-size:12px;text-transform:uppercase;letter-spacing:.12em;font-weight:900}.ba-box b{display:block;font-size:clamp(23px,2.6vw,38px);margin-top:4px}.ba-arrow{font-size:42px;color:#f7c948;font-weight:1000}.idle-title{font-size:clamp(50px,7vw,96px);font-weight:1000;letter-spacing:-.06em;color:#fff}.idle-sub{font-size:clamp(20px,2.5vw,32px);color:#cbd5e1;margin-top:16px}.ready-line{display:flex;gap:10px;flex-wrap:wrap;justify-content:center;margin-top:22px}.ready-chip{padding:8px 12px;border-radius:999px;background:rgba(148,163,184,.08);border:1px solid rgba(148,163,184,.16);color:#cbd5e1;font-size:12px;font-weight:900;letter-spacing:.06em}.ready-chip.ok{background:#083c2d;border-color:#1e7b5c;color:#a7f3d0}.test-badge{display:inline-block;margin-top:18px;padding:8px 12px;border-radius:999px;background:#3f2b0b;border:1px solid #8b6214;color:#fde68a;font-size:12px;font-weight:900;letter-spacing:.11em}.finale .idle-title{color:#f7c948}.finale-mark{font-size:clamp(74px,10vw,140px);line-height:1;margin-bottom:14px;filter:drop-shadow(0 14px 24px rgba(247,201,72,.18))}.loading{display:grid;place-items:center;min-height:700px;color:#fde68a;font-weight:1000;letter-spacing:.2em}
@media(max-width:780px){.gala-shell{padding:26px 16px;min-height:690px;border-radius:20px}.stage{min-height:610px}.nominee{grid-template-columns:1fr;gap:8px;text-align:center;padding:16px}.nominee-stats{justify-content:center}.vs-grid{grid-template-columns:1fr;gap:10px}.before-after{grid-template-columns:1fr;gap:8px}.ba-arrow{transform:rotate(90deg)}}
</style>
<script>
...
function chips(lines){return (lines||[]).map(x=>`<span class="stat-chip">${esc(x)}</span>`).join('')}
function winnerLines(lines){return `<div class="winner-lines">${(lines||[]).map(x=>`<div class="winner-line">${esc(x)}</div>`).join('')}</div>`}
function renderIdle(s){let selected=Number(s.selected_count||0),required=Number(s.required_count||18),missing=(s.missing_selection_details||[]).length,unavailable=(s.unavailable||[]).length;let sub=s.ready?'Laureaci wybrani. Czekamy na rozpoczęcie gali z telefonu.':'Próba techniczna jest dostępna, ale finalna gala nie jest jeszcze kompletna.';let chips=`<div class="ready-line"><span class="ready-chip ${s.ready?'ok':''}">${selected} / ${required} laureatów</span>${unavailable?`<span class="ready-chip">${unavailable} kategorii bez kandydatów</span>`:''}${missing?`<span class="ready-chip">${missing} wyborów do zamknięcia</span>`:''}</div>`;return `${topLine(s)}<div class="stage"><div class="idle-title">FIFA NIGHT<br>AWARDS ${esc(s.year)}</div><div class="idle-sub">${esc(sub)}</div>${chips}${s.test_mode?'<div class="test-badge">TRYB TESTOWY • BRAKI SĄ POMIJANE / UZUPEŁNIANE LIDEREM</div>':''}</div>`}
function renderIntro(s,c){
  const idx=Number(s.current_index)+1,total=s.total_categories,key=String(c.key||'');
  if(key==='player_year')return `${topLine(s)}<div class="stage cinematic-intro player-year-cinematic"><div class="cinematic-layer py-beat-1"><div class="eyebrow">KATEGORIA ${idx} / ${total}</div><h1 class="category">${esc(c.title||'')}</h1><div class="quote">Cały rok pierdolenia, losowań, karnych, comebacków i pretensji do gry sprowadza się do jednego pytania…</div></div><div class="cinematic-layer py-beat-2"><div class="cinematic-question">Kto był najlepszy?</div></div><div class="cinematic-layer py-beat-3"><div class="cinematic-final-lines"><div>Nie za jeden mecz.</div><div>Nie za jeden turniej.</div><strong>Za cały jebany sezon.</strong></div></div></div>`;
  if(key==='match_year')return `${topLine(s)}<div class="stage cinematic-intro match-year-cinematic"><div class="eyebrow cinematic-fixed">${esc(c.title||'')}</div><div class="cinematic-layer match-beat-1"><div class="cinematic-bigline">Były mecze dobre.</div></div><div class="cinematic-layer match-beat-2"><div class="cinematic-bigline">Były mecze dobre.</div><div class="cinematic-bigline accent-soft">Były mecze pojebane.</div></div><div class="cinematic-layer match-beat-3"><div class="cinematic-question">A potem był ten.</div></div></div>`;
  if(key==='late_king')return `${topLine(s)}<div class="stage cinematic-intro late-cinematic"><div class="eyebrow cinematic-fixed">${esc(c.title||'')}</div><div class="cinematic-layer late-beat-1"><div class="cinematic-clock">84:59</div></div><div class="cinematic-layer late-beat-2"><div class="cinematic-clock">88:00</div></div><div class="cinematic-layer late-beat-3"><div class="cinematic-clock danger">90:00</div></div><div class="cinematic-layer late-beat-4"><div class="quote cinematic-quote">Najgorszy moment, żeby przeciwko niemu uwierzyć, że już po meczu.</div></div></div>`;
  if(key==='progress')return `${topLine(s)}<div class="stage cinematic-intro progress-cinematic"><div class="eyebrow cinematic-fixed">${esc(c.title||'')}</div><div class="cinematic-layer progress-beat-1"><div class="progress-label">POCZĄTEK SEZONU</div><div class="progress-meter"><span class="low"></span></div></div><div class="cinematic-layer progress-beat-2"><div class="progress-label">KONIEC SEZONU</div><div class="progress-meter"><span class="high"></span></div></div><div class="cinematic-layer progress-beat-3"><div class="quote cinematic-quote">Na początku sezonu był mięsem armatnim. Potem coś się, kurwa, zmieniło.</div></div></div>`;
  return `${topLine(s)}<div class="stage intro"><div class="eyebrow">KATEGORIA ${idx} / ${total}</div><h1 class="category">${esc(c.title||'')}</h1><div class="quote">${esc(c.opening_text||'')}</div>${c.second_opening_text?`<div class="quote second">${esc(c.second_opening_text)}</div>`:''}</div>`
}
function renderNominees(s,c){let n=(c.nominees||[]).slice(0,Number(s.nominees_revealed||0));return `${topLine(s)}<div class="stage"><div class="eyebrow">NOMINOWANI</div><div class="nominees-title">${esc(c.title||'')}</div><div class="nominee-list">${n.map((x,i)=>{let g=x.gala||{};return `<div class="nominee" style="animation-delay:${i*.05}s"><div class="nominee-name">${esc(g.display_name||x.name||'—')}</div><div class="nominee-stats">${chips(g.teaser_lines||[])}</div></div>`}).join('')}</div></div>`}
function renderSuspense(s,c){return `${topLine(s)}<div class="stage suspense"><div class="eyebrow">${esc(c.title||'')}</div><div class="award-to">FIFA NIGHT AWARD ${esc(s.year)} TRAFIA DO</div><div class="line"></div><div class="dots">…</div></div>`}
function fmtMoney(v){let n=Number(v||0);return n.toLocaleString('pl-PL',{minimumFractionDigits:2,maximumFractionDigits:2})+' zł'}
function goalCounterHtml(g,phase){let vals=(g.special?.values||[]).map(Number);let final=vals.length?vals[vals.length-1]:0;return `<div class="goal-counter" id="goal-counter">${phase==='winner_animation'?'0':final} GOLI</div>`}
function beforeAfterHtml(sp,phase){let f=(v,signed=false)=>{let n=Number(v||0);let out=n.toLocaleString('pl-PL',{minimumFractionDigits:2,maximumFractionDigits:2});return signed&&n>0?'+'+out:out};return `<div class="before-after ${phase==='winner_animation'?'anim':''}"><div class="ba-box"><span>POCZĄTEK</span><b>${f(sp.points_before)} pkt/mecz<br>${f(sp.gd_before,true)} GD/mecz</b></div><div class="ba-arrow">→</div><div class="ba-box"><span>KOŃCÓWKA</span><b>${f(sp.points_after)} pkt/mecz<br>${f(sp.gd_after,true)} GD/mecz</b></div></div>`}
function vsHtml(g,c,key){let sp=g.special||{};let a,b;if(key==='rivalry'){let names=c.participant_names||[];a=names[0]||'?';b=names[1]||'?'}else{a=c.home_name||'?';b=c.away_name||'?'}let win=sp.winner_name||'';let lose=sp.loser_name||'';let badge=(nm)=>nm===win?'<div class="medal gold">GOLD</div>':(nm===lose?'<div class="medal transparent">TRANSPARENT</div>':'');let side=(nm,pos)=>`<div class="vs-side ${pos} ${nm===win?'is-winner':''}"><div class="vs-name">${esc(nm)}</div>${badge(nm)}</div>`;let center='';if(key==='rivalry'){let wa=Number(c.wins_a||0),wb=Number(c.wins_b||0);center=`<div><div class="rivalry-record">${wa} : ${wb}</div><div class="vs-mark">H2H</div></div>`}else{let hs=Number(c.home_score||0),as=Number(c.away_score||0),pens=(c.home_penalties!==null&&c.home_penalties!==undefined&&c.away_penalties!==null&&c.away_penalties!==undefined)?`<div class="match-pen">karne ${esc(c.home_penalties)} : ${esc(c.away_penalties)}</div>`:'';center=`<div><div class="match-score">${hs} : ${as}</div>${pens}<div class="vs-mark">VS</div></div>`}return `<div class="vs-grid">${side(a,'left')}${center}${side(b,'right')}</div>`}
function renderWinner(s,c){let w=c.winner||{name:'—',gala:{}},g=w.gala||{},sp=g.special||{},phase=s.display_phase,cls=phase==='winner_animation'?'winner-anim':'winner-hold';let special='',specialVs=(sp.type==='rivalry_vs'||sp.type==='match_vs');if(sp.type==='goal_progress')special=goalCounterHtml(g,phase);else if(sp.type==='before_after')special=beforeAfterHtml(sp,phase);else if(sp.type==='finance_equation')special=`<div class="equation ${phase==='winner_animation'?'anim':''}"><span>${fmtMoney(sp.won)}</span><span>−</span><span>${fmtMoney(sp.paid)}</span><span>=</span><span class="result">${Number(sp.balance||0)>=0?'+':''}${fmtMoney(sp.balance)}</span></div>`;else if(specialVs)special=vsHtml(g,w,c.key);else if(sp.type==='late_clock')special=`<div class="clock" id="late-clock">${phase==='winner_animation'?'85:00':esc(sp.latest||'90+')}</div>`;else if(sp.type==='player_year')special=`<div class="hero-metric ${phase==='winner_animation'?'anim':''}">${esc(sp.hero_metric||'')}</div>`;let lines=(g.winner_lines||[]);if(specialVs)lines=lines.slice(1);return `${topLine(s)}<div class="stage ${cls}"><div class="winner-card ${specialVs?'special-vs-card':''}"><div class="winner-label">${esc(plainTitle(c.title||''))} • ${esc(s.year)}</div>${specialVs?'':`<div class="winner-name">${esc(g.display_name||w.name||'—')}</div>`}${special}${winnerLines(lines)}<div class="hold-note">${phase==='winner_hold'?'CZEKA NA NASTĘPNĄ KATEGORIĘ':'ODSŁANIAMY LAUREATA'}</div></div></div>`}
...
function visualPhase(s){return s?.display_phase==='intro_pending'?'intro':(s?.display_phase||'')}
async function poll(){try{let r=await fetch(ENDPOINT,{cache:'no-store'});if(r.ok){let next=await r.json();let sig=[next.status,next.current_index,visualPhase(next),next.nominees_revealed].join('|');let old=[state.status,state.current_index,visualPhase(state),state.nominees_revealed].join('|');if(sig!==old)render(next);else{state=next;if(next.display_phase==='intro_pending')ackTvReady(next)}}}catch(e){}}
