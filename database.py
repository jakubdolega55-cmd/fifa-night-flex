...
class Database:
    ...
    def _extra_for_format(self, format_key: str, rng: random.Random) -> dict:
        if format_key == "double5": return {"bye_winner_match": rng.choice([1,2])}
        if format_key == "double7": return {"lb_bye_match": 3}
        return {}
    ...
    def create_tournament(self, player_names: list[str], player_count: int, format_key: str, teams: list[str], is_test: bool) -> str:
        ...
        if player_count == 5 and format_key != "double5": raise ValueError("Nieprawidłowy format dla 5 graczy.")
        if player_count == 6 and format_key != "groups6": raise ValueError("Nieprawidłowy format dla 6 graczy.")
        if player_count == 7 and format_key not in ("double7", "groups7"): raise ValueError("Wybierz format turnieju 7-osobowego.")
    ...
    def _apply_draw_groups_conn(self, conn, tid: str, format_key: str, draw: dict) -> None:
        ...
        if format_key in ("groups6", "groups7"):
            for g in ("A","B"):
    ...
    def undo_last_result(self, tid: str) -> int | None:
        ...
        with self.connect() as conn:
            ...
            meta=self._fetchone(conn,"SELECT format_key FROM flex_tournament_meta WHERE tournament_id=?",(tid,)); self._resolve_all_conn(conn,tid,meta["format_key"])
            return no

    def standings(self, tid: str) -> dict[str,list[dict]]:
        with self.connect() as conn:
            meta=self._fetchone(conn,"SELECT format_key FROM flex_tournament_meta WHERE tournament_id=?",(tid,)); fmt=meta["format_key"]
            if fmt=="league4_final": return {"L":self._table_from_conn(conn,tid,"L")}
            if fmt in ("groups6","groups7"): return {"A":self._table_from_conn(conn,tid,"A"),"B":self._table_from_conn(conn,tid,"B")}
            return {}
