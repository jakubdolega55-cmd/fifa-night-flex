...
# Gala order is deliberately the reverse of the organizer's winner-selection flow:
# lighter/statistical awards first, Player of the Year as the finale.
GALA_AWARD_ORDER = [
    "superscorer", "team_best", "finance", "debut", "progress", "outsider",
    "rivalry", "fair_play", "universal", "comeback_king", "late_king",
    "spectacle", "match_year", "clutch", "player_scorers", "defense",
    "offensive", "player_year",
]
GALA_OPENING_TEXTS = {
    "superscorer": "Nieważne kto trzymał pada. Ten gość po prostu napierdalał gole.",
    "team_best": "Najbardziej opłacalny los na kole.",
    "finance": "Nie wiadomo, czy grał w FIFĘ, czy inwestował.",
    "debut": "Jedni potrzebowali czasu. Inni od pierwszych meczów zachowywali się, jakby grali tu od lat.",
    "progress": "Na początku sezonu był mięsem armatnim. Potem coś się, kurwa, zmieniło.",
    "outsider": "Bez gabloty pełnej pucharów. Za to z wyjątkowym talentem do wkurwiania faworytów.",
    "rivalry": "Losowanie miało wiele opcji. I jakoś ciągle wybierało ich.",
    "fair_play": "Da się podobno wygrać bez koszenia wszystkiego, co się rusza. Mamy dowody.",
    "universal": "Nie pytał, kim gra. Pytał, z kim.",
    "comeback_king": "Przegrywać też trzeba umieć. Zwłaszcza przez pierwsze 70 minut.",
    "late_king": "Najgorszy moment, żeby przeciwko niemu uwierzyć, że już po meczu.",
    "spectacle": "Defensywa była opcjonalna. Rozrywka obowiązkowa.",
    "match_year": "Były mecze dobre. Były mecze pojebane. A potem był ten.",
    "clutch": "Presja? Dla jednych problem. Dla innych sygnał START.",
    "player_scorers": "Obrońcy rywali znali go już zdecydowanie zbyt dobrze.",
    "defense": "Tu nie było autobusu. Tu był parking wielopoziomowy.",
    "offensive": "Obrona? Jest od tego drugi pad.",
    "player_year": "Cały rok pierdolenia, losowań, karnych, comebacków i pretensji do gry sprowadza się do jednego pytania… Kto był najlepszy?",
}
...
# During implementation/testing the Gala can be opened before all organizer
...
GALA_AWARD_LABELS = {
    ...
    "rivalry": "Rywalizacja Roku", "fair_play": "Fair Play", "universal": "Najbardziej Uniwersalny Gracz",
    "comeback_king": "Comeback King", "late_king": "Król Końcówek", "spectacle": "Najbardziej Widowiskowy Gracz",
    "match_year": "Mecz Roku", "clutch": "Clutch Player Roku", "player_scorers": "Król Strzelców FIFA Night",
    "defense": "Beton Roku", "offensive": "Ofensywny Gracz Roku", "player_year": "Gracz Roku",
}
...
class Database:
    ...
    def init_schema(self) -> None:
        stmts = [
            ...
            """CREATE TABLE IF NOT EXISTS match_summary_stats (
                tournament_id TEXT NOT NULL, match_no INTEGER NOT NULL, game_version TEXT NOT NULL,
                parser_version TEXT NOT NULL, stats_json TEXT NOT NULL, confidence_json TEXT NOT NULL DEFAULT '{}',
    ...
    def _migrate_double_elim_single_final_conn(self, conn) -> None:
        ...
        for row in rows:
            ...
            conn.execute(self._sql("DELETE FROM match_events WHERE tournament_id=? AND match_no=?"),(tid,reset_no)); conn.execute(self._sql("DELETE FROM match_summary_stats WHERE tournament_id=? AND match_no=?"),(tid,reset_no))
    ...
    def upsert_match_summary_stats(self, tid: str, match_no: int, game_version: str, parser_version: str, stats: dict, confidence: dict | None = None) -> None:
        """Store one parsed EA FC Summary snapshot without retaining the source image.

        scan-preview may call this before the result is confirmed. Annual Awards only
        consume rows whose real match result exists and is not a forfeit. Re-scanning
        the same match replaces the snapshot atomically.
        """
        stamp=now_iso()
        with self.connect() as conn:
            conn.execute(self._sql("""
                INSERT INTO match_summary_stats (
                    tournament_id,match_no,game_version,parser_version,stats_json,confidence_json,created_at,updated_at
                ) VALUES (?,?,?,?,?,?,?,?)
                ON CONFLICT(tournament_id,match_no) DO UPDATE SET
                    game_version=excluded.game_version, parser_version=excluded.parser_version,
                    stats_json=excluded.stats_json, confidence_json=excluded.confidence_json,
                    updated_at=excluded.updated_at
            """),(
                str(tid),int(match_no),normalize_game_version(game_version),str(parser_version or "summary-v1"),
                json.dumps(stats or {},ensure_ascii=False),json.dumps(confidence or {},ensure_ascii=False),stamp,stamp
            ))

    def match_summary_stats(self, tid: str, match_no: int) -> dict | None:
        with self.connect() as conn:
            row=self._fetchone(conn,"""
                SELECT tournament_id,match_no,game_version,parser_version,stats_json,confidence_json,created_at,updated_at
                FROM match_summary_stats WHERE tournament_id=? AND match_no=?
            """,(str(tid),int(match_no)))
        if not row:return None
        try:row["stats"]=json.loads(row.pop("stats_json") or "{}")
        except Exception:row["stats"]={}
        try:row["confidence"]=json.loads(row.pop("confidence_json") or "{}")
        except Exception:row["confidence"]={}
        return row
    ...
    def forfeit_match(self, tid: str, match_no: int, forfeiting_player_id: str) -> dict:
        ...
        with self.connect() as conn:
            ...
            conn.execute(self._sql("DELETE FROM match_events WHERE tournament_id=? AND match_no=?"),(tid,int(match_no))); conn.execute(self._sql("DELETE FROM match_summary_stats WHERE tournament_id=? AND match_no=?"),(tid,int(match_no)))
    ...
    def reset_current(self, tid: str) -> None:
        with self.connect() as conn:
            conn.execute(self._sql("DELETE FROM flex_match_sources WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM match_scorers WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM tournament_absences WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM match_events WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM match_summary_stats WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM matches WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM tournament_players WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM flex_tournament_meta WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM tournaments WHERE id=?"),(tid,)); self._setting_set_conn(conn,CURRENT_KEY,"")
    ...
    def clear_flex_history(self) -> None:
        with self.connect() as conn:
            ...
            for tid in ids:
                conn.execute(self._sql("DELETE FROM flex_match_sources WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM match_scorers WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM tournament_absences WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM match_events WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM match_summary_stats WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM matches WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM tournament_players WHERE tournament_id=?"),(tid,)); conn.execute(self._sql("DELETE FROM tournaments WHERE id=?"),(tid,))
    ...
    def clear_all_history(self) -> None:
        with self.connect() as conn:
            ...
            conn.execute("DELETE FROM match_events"); conn.execute("DELETE FROM match_summary_stats")
            conn.execute("DELETE FROM flex_tournament_meta")
    ...
    def awards_gala_nav_visible(self, year: int) -> bool:
        """Cheap navigation gate; the full readiness validation still happens at START."""
        if GALA_TEST_MODE:
            return True
        with self.connect() as conn:
            raw=self._setting_get_conn(conn,f"flex_award_selections_{int(year)}")
        try:
            selections=json.loads(raw) if raw else {}
        except Exception:
            selections={}
        return all(bool((selections.get(key) or {}).get("id")) for key in GALA_AWARD_ORDER)

    def _gala_load_state(self, year: int) -> dict:
        with self.connect() as conn:
            raw=self._setting_get_conn(conn,self._gala_state_key(year))
        try:
    ...
    def _gala_save_state(self, year: int, state: dict) -> None:
        with self.connect() as conn:
            self._setting_set_conn(conn,self._gala_state_key(year),json.dumps(state,ensure_ascii=False))

    def _gala_base(self, year: int) -> dict:
        data=self.annual_awards(int(year))
        categories={str(c.get("key") or ""):c for c in (data.get("categories") or []) if c.get("award") is not False}
        selections=data.get("selections") or {}
        available=[key for key in GALA_AWARD_ORDER if key in categories and (categories[key].get("candidates") or [])]
        unavailable=[key for key in GALA_AWARD_ORDER if key not in available]
        invalid=[];missing=[];valid_selected=[]
        for key in available:
            sid=str((selections.get(key) or {}).get("id") or "")
            valid_ids={str(x.get("id") or "") for x in ((categories.get(key) or {}).get("candidates") or [])}
            if not sid:missing.append(key)
            elif sid not in valid_ids:invalid.append(key);missing.append(key)
            else:valid_selected.append(key)
        ready=(not unavailable and not missing and len(available)==len(GALA_AWARD_ORDER))
        selected_count=len(valid_selected)
        details=lambda keys:[{"key":key,"label":GALA_AWARD_LABELS.get(key,key)} for key in keys]
        return {
            "awards":data,"categories":categories,"selections":selections,"available_keys":available,
            "unavailable_keys":unavailable,"unavailable":details(unavailable),
            "missing_selections":missing,"missing_selection_details":details(missing),
            "invalid_selections":invalid,"invalid_selection_details":details(invalid),
            "ready":ready,"selected_count":selected_count,"required_count":len(GALA_AWARD_ORDER),
            "available":bool(GALA_TEST_MODE or ready),"test_mode":bool(GALA_TEST_MODE),
        }

    def _gala_nominee_order(self, base: dict, order: list[str] | None = None) -> dict:
        selections=base.get("selections") or {}; orders={}
        rng=random.SystemRandom()
        for key in (order or GALA_AWARD_ORDER):
            cat=(base.get("categories") or {}).get(key) or {}
            candidates=list(cat.get("candidates") or [])
            top3=candidates[:3]
            selected_id=str((selections.get(key) or {}).get("id") or "")
            if selected_id and all(str(x.get("id") or "")!=selected_id for x in top3):
                picked=next((x for x in candidates if str(x.get("id") or "")==selected_id),None)
                if picked:
                    top3=(top3[:2]+[picked]) if len(top3)>=3 else (top3+[picked])
            ids=[str(x.get("id") or "") for x in top3 if str(x.get("id") or "")]
            rng.shuffle(ids);orders[key]=ids
        return orders

    def _gala_winner_ids(self, base: dict, order: list[str]) -> dict:
        out={}
        selections=base.get("selections") or {}
        categories=base.get("categories") or {}
        for key in order:
            candidates=list((categories.get(key) or {}).get("candidates") or [])
            candidate_ids={str(x.get("id") or "") for x in candidates}
            selected_id=str((selections.get(key) or {}).get("id") or "")
            if selected_id and selected_id in candidate_ids:
                out[key]=selected_id
                continue
            # Test mode may run before organizer selections are complete (or after an
            # old selection became stale). Freeze the current statistical leader at
            # START so refreshes/edits cannot change a winner mid-rehearsal.
            if GALA_TEST_MODE and candidates and candidates[0].get("id") is not None:
    ...
    def advance_awards_gala(self, year: int) -> dict:
        state=self._gala_load_state(year)
        if not state or state.get("status") in (None,"idle","finished"):
            return self.start_awards_gala(year)
        if state.get("status")=="finale":
            state["status"]="finished";state["completed_at"]=now_iso();state["updated_at"]=now_iso();self._gala_save_state(year,state)
            return self.awards_gala_status(year)
        live=self.awards_gala_status(year)
        if live.get("display_phase")!="winner_hold":
            raise ValueError("Prezentacja kategorii jeszcze trwa. Następna kategoria odblokuje się po revealu laureata.")
        order=list(state.get("order") or GALA_AWARD_ORDER)
    ...
    def replay_awards_gala_category(self, year: int) -> dict:
        state=self._gala_load_state(year)
        if not state or state.get("status")!="running":
            raise ValueError("Gala nie jest aktualnie w kategorii do powtórzenia.")
        live=self.awards_gala_status(year)
        if live.get("display_phase")!="winner_hold":
    ...
    def reset_awards_gala(self, year: int) -> dict:
        state={"year":int(year),"status":"idle","current_index":-1,"order":[],"nominee_order":{},"winner_ids":{},"updated_at":now_iso()}
        self._gala_save_state(year,state)
        return self.awards_gala_status(year)

    @staticmethod
    def _gala_candidate_presentation(key: str, cand: dict | None) -> dict:
        """Public gala copy for one nominee/winner.

        TOP3 deliberately hides the strongest deciding metric in most categories.
        The winner card can reveal all ranking inputs. This payload is shared by TV
        now and can be reused by the phone controller later.
        """
        c=dict(cand or {}); key=str(key or "")
        name=str(c.get("name") or "—")
        teaser=[]; winner=[]; special={}
        def pct(v):
            try:return f"{float(v):.1f}%".replace(".0%","%")
            except Exception:return "—"
        def num(v,d=2):
            try:
                x=float(v); out=f"{x:.{d}f}"
                return out.replace(".",",")
            except Exception:return "—"
        def signed(v,d=0):
            try:
        ...
        if key=="superscorer":
            ...
            teaser=[f"{money(c.get('won_pln'))} wygranych"]
            winner=[f"wygrane {money(c.get('won_pln'))}",f"wpłaty {money(c.get('paid_pln'))}",f"bilans {signed(c.get('balance_pln'),2)} zł"]
            special={"type":"finance_equation","won":c.get("won_pln"),"paid":c.get("paid_pln"),"balance":c.get("balance_pln")}
        elif key=="debut":
            teaser=[f"{pct(c.get('win_pct'))} W",f"{int(c.get('w') or 0)} : {int(c.get('d') or 0)} : {int(c.get('l') or 0)}   W • R • P"]
            winner=[f"pierwsze {int(c.get('matches') or 0)} meczów • {int(c.get('w') or 0)} W / {int(c.get('d') or 0)} R / {int(c.get('l') or 0)} P",
                    f"{pct(c.get('win_pct'))} W • {num(c.get('points_per_match'),2)} pkt/mecz",
                    f"bramki {int(c.get('gf') or 0)}:{int(c.get('ga') or 0)} • bilans {signed(c.get('gd'),0)}",
                    f"{int(c.get('finals') or 0)} finały • {int(c.get('titles') or 0)} tytuły"]
        elif key=="progress":
            teaser=[f"{int(c.get('matches') or 0)} meczów • {pct(c.get('win_pct'))} W"]
            winner=[f"punkty/mecz  {num(c.get('early_points_per_match'),2)} → {num(c.get('late_points_per_match'),2)}",
                    f"bilans bramek/mecz  {signed(c.get('early_gd_per_match'),2)} → {signed(c.get('late_gd_per_match'),2)}"]
            special={"type":"before_after","points_before":c.get("early_points_per_match"),"points_after":c.get("late_points_per_match"),
            ...
            if int(c.get("semifinals") or 0):detail.append(f"{int(c.get('semifinals') or 0)} półfinał(y)")
            teaser=[f"{int(c.get('matches') or 0)} bezpośrednich spotkań",(" • ".join(detail) if detail else "turniejowe H2H")]
            names=list(c.get("participant_names") or [])
            a=names[0] if len(names)>0 else "?"; b=names[1] if len(names)>1 else "?"
            winner=[f"{a} {int(c.get('wins_a') or 0)} : {int(c.get('wins_b') or 0)} {b}",
                    f"bramki {int(c.get('goals_a') or 0)}:{int(c.get('goals_b') or 0)} • {int(c.get('matches') or 0)} meczów"]
            if detail:winner.append(" • ".join(detail))
            if int(c.get("penalty_matches") or 0):winner.append(f"{int(c.get('penalty_matches') or 0)} mecz(e) po karnych")
            decider=str(c.get("winner_decider") or "").strip()
            if decider and decider!="zwycięstwa H2H":winner.append(f"rozstrzygnięcie: {decider}")
            special={"type":"rivalry_vs","winner_name":c.get("winner_name"),"loser_name":c.get("loser_name"),"decider":c.get("winner_decider")}
        elif key=="fair_play":
            teaser=[f"{int(c.get('matches') or 0)} meczów liczonych",(f"{int(c.get('fouls_total'))} fauli" if c.get("fouls_total") is not None else "faule: brak Summary")]
            winner=[f"{num(c.get('discipline_per_match'),2)} pkt dyscypliny/mecz",f"🟨 {int(c.get('yellow') or 0)} • 🟥 {int(c.get('red') or 0)}"]
            ...
            winner=[f"{num(c.get('avg_goals'),2)} gola/mecz w jego spotkaniach",f"{int(c.get('spectacular_matches') or 0)}/{int(c.get('matches') or 0)} bardzo widowiskowych",
                    f"{int(c.get('penalty_matches') or 0)} mecze z karnymi"]
            if c.get("summary_total_xg_per_match") is not None:winner.append(f"łączne xG {num(c.get('summary_total_xg_per_match'),2)}/mecz")
            if c.get("summary_total_shots_per_match") is not None:winner.append(f"strzały {num(c.get('summary_total_shots_per_match'),1)}/mecz")
        elif key=="match_year":
            hn=str(c.get("home_name") or "?"); an=str(c.get("away_name") or "?")
            name=f"{hn} ⚔️ {an}"
            teaser=[str(c.get("stage_label") or "mecz")]
            if float(c.get("cash_pot_pln") or 0)>0:teaser.append(f"pula {money(c.get('cash_pot_pln'))}")
            result=f"{hn} {int(c.get('home_score') or 0)} : {int(c.get('away_score') or 0)} {an}"
            if c.get("home_penalties") is not None and c.get("away_penalties") is not None:result+=f"  (k. {c.get('home_penalties')}:{c.get('away_penalties')})"
            winner=[result,str(c.get("stage_label") or "mecz")]
            if float(c.get("cash_pot_pln") or 0)>0:winner.append(f"pula {money(c.get('cash_pot_pln'))}")
            if c.get("drama_text"):winner.append(" • ".join(str(x) for x in c.get("drama_text") or []))
            if c.get("summary_total_xg") is not None:winner.append(f"łączne xG {num(c.get('summary_total_xg'),2)}")
            if c.get("summary_total_shots") is not None:winner.append(f"{int(c.get('summary_total_shots') or 0)} strzałów")
            ...
            other=sum(int(v.get("m") or 0) for k,v in by.items() if k not in ("FINAL","RESET_FINAL","SF")); ow=sum(int(v.get("w") or 0) for k,v in by.items() if k not in ("FINAL","RESET_FINAL","SF"))
            parts=[]
            if finals:parts.append(f"finały {fw}/{finals}")
            if semis:parts.append(f"półfinały {sw}/{semis}")
            if other:parts.append(f"pozostałe {ow}/{other}")
            if parts:winner.append(" • ".join(parts))
        elif key=="player_scorers":
            teaser=[f"{int(c.get('matches') or 0)} meczów • {int(c.get('hattricks') or 0)} hat-tricki"]
            ...
            teaser=[f"{int(c.get('starts') or 0)} turniejów • {int(c.get('matches') or 0)} meczów • {int(c.get('finals') or 0)} finałów",
                    ...
                    f"bramki {int(c.get('gf') or 0)}:{int(c.get('ga') or 0)} • clutch {int(c.get('clutch_wins') or 0)}/{int(c.get('clutch_matches') or 0)}"]
            winner=[f"{int(c.get('starts') or 0)} turniejów • {int(c.get('matches') or 0)} meczów • {pct(c.get('win_pct'))} W",
                    f"{int(c.get('wins') or 0)} W • {int(c.get('draws') or 0)} R • {int(c.get('losses') or 0)} P",
                    f"bramki {int(c.get('gf') or 0)}:{int(c.get('ga') or 0)} • bilans {signed(c.get('gd'),0)}",
                    f"clutch {int(c.get('clutch_wins') or 0)}/{int(c.get('clutch_matches') or 0)} • {pct(c.get('clutch_pct'))}"]
            if c.get("summary_xgd_per_match") is not None:winner.append(f"xGD {signed(c.get('summary_xgd_per_match'),2)}/mecz")
            special={"type":"player_year","hero_metric":f"{int(c.get('titles') or 0)} TYTUŁY Z {int(c.get('finals') or 0)} FINAŁÓW"}
        else:
            teaser=[str(c.get("reason") or "")]
            winner=list(teaser)
    ...
    def awards_gala_status(self, year: int) -> dict:
        ...
        runtime_order=saved_order or (list(GALA_AWARD_ORDER) if status in ("running","finale","finished") else list(GALA_AWARD_ORDER))
        current_key=runtime_order[idx] if status=="running" and 0<=idx<len(runtime_order) else None
        ...
        if current_key:
            ...
            if not started_raw:display_phase="intro_pending"
                ...
                display_phase="suspense";reveal_count=len(nominees)
            elif elapsed<intro+nom+sus+anim:
                display_phase="winner_animation";reveal_count=len(nominees)
            else:
                display_phase="winner_hold";reveal_count=len(nominees)
            _nominees=[]
            for _n in nominees:
                _x=dict(_n);_x["gala"]=self._gala_candidate_presentation(current_key,_n);_nominees.append(_x)
            _winner=dict(winner) if winner else None
            if _winner is not None:_winner["gala"]=self._gala_candidate_presentation(current_key,_winner)
            current={
                "key":current_key,"title":cat.get("title") or GALA_AWARD_LABELS.get(current_key,current_key),"engraving":cat.get("engraving"),
                "opening_text":GALA_OPENING_TEXTS.get(current_key,""),
                "second_opening_text":GALA_PLAYER_YEAR_SECOND_LINE if current_key=="player_year" else None,
                "nominees":_nominees,"winner":_winner,"winner_is_test_fallback":fallback,
            }
        winner_summary=[]
        ...
        return {
            ...
            "next_label":("ZAKOŃCZ GALĘ" if status=="running" and idx==total-1 else ("ZAMKNIJ GALĘ" if status=="finale" else "NASTĘPNA KATEGORIA")),
        }
    ...
    def annual_awards(self, year: int) -> dict:
        ...
        with self.connect() as conn:
            ...
            detailed_event_rows=self._fetchall(conn,f"""SELECT me.tournament_id,me.match_no,me.event_order,me.event_type,
                ...
                ORDER BY me.tournament_id,me.match_no,me.event_order,me.id""",tuple(tids))
            summary_rows=self._fetchall(conn,f"""SELECT ms.tournament_id,ms.match_no,ms.game_version,ms.parser_version,ms.stats_json,ms.confidence_json
                FROM match_summary_stats ms JOIN matches mm ON mm.tournament_id=ms.tournament_id AND mm.match_no=ms.match_no
                WHERE ms.tournament_id IN ({q}) AND mm.home_score IS NOT NULL AND COALESCE(mm.match_status,'played')<>'forfeit'
                ORDER BY ms.tournament_id,ms.match_no""",tuple(tids))
        ...
        ps=defaultdict(lambda:{"m":0,"w":0,"d":0,"l":0,"gf":0,"ga":0,"clean_sheets":0,"titles":0,"finals":0,"clutch_m":0,"clutch_w":0,
                               ...
                               "clutch_by_stage":defaultdict(lambda:{"m":0,"w":0}),
        ...
        pair=defaultdict(lambda:{
            "n":0,"aw":0,"bw":0,"d":0,"goals_a":0,"goals_b":0,
            "important_matches":0,"importance_points":0,"finals":0,"semifinals":0,"knockouts":0,
            "stage_wins_a":defaultdict(int),"stage_wins_b":defaultdict(int),
        ...
        # a real, non-forfeit result above can influence Awards. Missing historical
        # Summary never counts as a zero.
        summary_by_match={}
        for _sr in summary_rows:
            try:_stats=json.loads(_sr.get("stats_json") or "{}")
            except Exception:_stats={}
            try:_conf=json.loads(_sr.get("confidence_json") or "{}")
            except Exception:_conf={}
            if not isinstance(_stats,dict):_stats={}
            summary_by_match[(str(_sr.get("tournament_id") or ""),int(_sr.get("match_no") or 0))]={"stats":_stats,"confidence":_conf}
        summary_ps=defaultdict(lambda:{
            "summary_matches":0,
        ...
        for m in matches:
            ...
            # Rywalizacja Roku deliberately ignores standalone 1 VS 1. It is a
            # season-long tournament rivalry, while duels keep their own ranking.
            rec=None
            if tid in tournament_ids:
                ...
                if h==k[0]:rec["goals_a"]+=hs;rec["goals_b"]+=ass
                else:rec["goals_a"]+=ass;rec["goals_b"]+=hs
                importance_weight=4 if stage in ("FINAL","RESET_FINAL") else (3 if stage in ("SF","WB_FINAL","LB_FINAL") else (2 if stage in ("QF","BARRAGE","WB","LB") else 0))
                if importance_weight:
                    rec["important_matches"]+=1;rec["importance_points"]+=importance_weight
                if stage in ("FINAL","RESET_FINAL"):rec["finals"]+=1
                elif stage in ("SF","WB_FINAL","LB_FINAL"):rec["semifinals"]+=1
                elif stage in ("QF","BARRAGE","WB","LB"):rec["knockouts"]+=1
                if rk=="W":rec["stage_wins_a"][importance_weight]+=1
                elif rk=="L":rec["stage_wins_b"][importance_weight]+=1
                if m.get("home_penalties") is not None and m.get("away_penalties") is not None:rec["penalty_matches"]+=1
                # Direct-H2H cards are the final sporting tie-break inside a selected rivalry.
                for _pe in moty_events_by_match.get((tid,int(m.get("match_no") or 0)),[]):
                    _et=str(_pe.get("event_type") or "");_actor=str(_pe.get("actor_player_id") or "")
                    _fp=1 if _et=="yellow_card" else (3 if _et=="red_card" else 0)
                    if not _fp:continue
                    if _actor==k[0]:rec["fairplay_a"]+=_fp
                    elif _actor==k[1]:rec["fairplay_b"]+=_fp
            ...
            # Optional Summary layer. Only trusted HOME/AWAY-mapped snapshots are
            # aggregated; missing values are ignored rather than treated as zero.
            _sm=summary_by_match.get((tid,int(m.get("match_no") or 0)))
            if _sm and bool((_sm.get("stats") or {}).get("usable_for_awards")):
                _sstats=_sm.get("stats") or {}; _home=_sstats.get("home") or {}; _away=_sstats.get("away") or {}
                def _num(_v):
                    try:
                        if _v is None or isinstance(_v,bool):return None
                        return float(_v)
                    except Exception:return None
                _feature={};_any=False
                for _pid,_mine,_opp in ((h,_home,_away),(a,_away,_home)):
                    _sp=summary_ps[_pid]
                    _xgf=_num(_mine.get("expected_goals"));_xga=_num(_opp.get("expected_goals"))
                    if _xgf is not None and _xga is not None:
                        _sp["xg_for"]+=_xgf;_sp["xg_against"]+=_xga;_sp["xg_n"]+=1;_any=True
                    _shf=_num(_mine.get("shots"));_sha=_num(_opp.get("shots"))
                    if _shf is not None and _sha is not None:
                        _sp["shots_for"]+=_shf;_sp["shots_against"]+=_sha;_sp["shots_n"]+=1;_any=True
                ...
                if _any:
                    summary_ps[h]["summary_matches"]+=1;summary_ps[a]["summary_matches"]+=1
                _hxg=_num(_home.get("expected_goals"));_axg=_num(_away.get("expected_goals"))
                if _hxg is not None and _axg is not None:_feature["total_xg"]=_hxg+_axg
                _hsh=_num(_home.get("shots"));_ash=_num(_away.get("shots"))
                if _hsh is not None and _ash is not None:_feature["total_shots"]=_hsh+_ash
                _hso=_num(_home.get("shots_on_target"));_aso=_num(_away.get("shots_on_target"))
                if _hso is not None and _aso is not None:_feature["total_sot"]=_hso+_aso
                if _feature:
                    summary_match_features[(tid,int(m.get("match_no") or 0))]=_feature
                    _xgv=min(max(float(_feature.get("total_xg") or 0.0),0.0)/6.0,1.0) if "total_xg" in _feature else 0.0
                    _shv=min(max(float(_feature.get("total_shots") or 0.0),0.0)/30.0,1.0) if "total_shots" in _feature else 0.0
                    _sov=min(max(float(_feature.get("total_sot") or 0.0),0.0)/14.0,1.0) if "total_sot" in _feature else 0.0
                    _sbonus=_xgv*.04+_shv*.025+_sov*.015
                    for _pid in (h,a):
                        summary_ps[_pid]["spectacle_bonus_sum"]+=_sbonus
                        summary_ps[_pid]["spectacle_n"]+=1
            ...
            for pid,team,gf,ga,r in ((h,m.get("home_team"),hs,ass,rh),(a,m.get("away_team"),ass,hs,ra)):
                ...
                if stage in clutch_stages:
                    ...
                    _cs=v["clutch_by_stage"][stage];_cs["m"]+=1;_cs["w"]+=int(r=="W")
                if r=="W":
            ...
            if rec is not None:
                # Small rivalry-drama layer, capped later at +10 for the whole pair.
                # It can separate otherwise similar rivalries but never dominate H2H volume/balance/stakes.
                _pair_drama=0.0
                if has_pens:_pair_drama+=1.5
                elif margin<=1:_pair_drama+=.75
                elif margin==2:_pair_drama+=.25
                _pair_drama+=min(1.25,drama_bonus/.06*1.25) if drama_bonus>0 else 0.0
                rec["drama_raw"]+=_pair_drama

            # Summary has a deliberately small Match of the Year bonus. A lively
            # statistical match can edge another similar candidate, but cannot
            # overpower score drama, elimination stakes or a final.
            _sf=summary_match_features.get(_mk) or {}
            _sxg=min(max(float(_sf.get("total_xg") or 0.0),0.0)/6.0,1.0) if "total_xg" in _sf else 0.0
            _ssh=min(max(float(_sf.get("total_shots") or 0.0),0.0)/30.0,1.0) if "total_shots" in _sf else 0.0
            _sso=min(max(float(_sf.get("total_sot") or 0.0),0.0)/14.0,1.0) if "total_sot" in _sf else 0.0
            summary_bonus=min(.04,_sxg*.020+_ssh*.012+_sso*.008)
            match_score=(
                closeness*.35
                + goals_value*.22
                + stakes*.18
            ...
            if "total_xg" in _sf:reason_parts.append(f"xG {_sf['total_xg']:.2f}")
            if "total_shots" in _sf:reason_parts.append(f"{int(_sf['total_shots'])} strzałów")
            ...
            _winner_pid=str(m.get("winner_player_id") or "")
            _loser_pid=(a if _winner_pid==h else (h if _winner_pid==a else None))
            match_candidates.append({
                ...
                "participant_ids":[h,a],
                "home_player_id":h,"away_player_id":a,"home_name":m.get("home_name"),"away_name":m.get("away_name"),
                "home_score":int(raw_hs),"away_score":int(raw_ass),"stage":stage,"stage_label":stage_text,
                "stakes_label":stakes_text,"cash_pot_pln":round(cash_pot_cents/100.0,2),
                "home_penalties":m.get("home_penalties"),"away_penalties":m.get("away_penalties"),
                "winner_player_id":_winner_pid or None,"winner_name":name_by.get(_winner_pid) if _winner_pid else None,
                "loser_player_id":_loser_pid,"loser_name":name_by.get(_loser_pid) if _loser_pid else None,
                "real_goals":int(hs+ass),"drama_text":list(drama_text),
                "summary_total_xg":(_sf.get("total_xg") if "total_xg" in _sf else None),
                "summary_total_shots":(_sf.get("total_shots") if "total_shots" in _sf else None),
            })
        ...
        scorer_display={}
        scorer_players=defaultdict(set)
        scorer_hattricks=defaultdict(int)
        scorer_goals_by_match=defaultdict(lambda:defaultdict(int))
        ...
        def _summary_strength(pid: str, metric_n: int) -> float:
            """Coverage/sample dampener for optional Summary data.

            Full force requires both broad season coverage and roughly ten parsed
            matches. Old matches without Summary are neutral, never treated as zero.
            """
            total=max(1,int(ps.get(pid,{}).get("m") or 0))
            n=max(0,int(metric_n or 0))
            return min(1.0,n/total)*min(1.0,n/10.0)
        def _summary_avg(pid: str, key: str, nkey: str) -> float | None:
            sp=summary_ps.get(pid) or {};n=int(sp.get(nkey) or 0)
            return (float(sp.get(key) or 0.0)/n) if n>0 else None
        def cand(pid,score,reason,**extra):
            out={"id":str(pid),"name":name_by.get(str(pid),"?"),"score":round(float(score),2),"reason":reason}
            out.update(extra)
            return out
        def top(items,n=5):
        ...
        for pid,v in ps.items():
            ...
            if xn>0:
                xgdpm=(float(sp.get("xg_for") or 0)-float(sp.get("xg_against") or 0))/xn
                strength=_summary_strength(pid,xn)
                score+=max(-1.5,min(1.5,xgdpm))*2.0*strength
                reason+=f" • xGD/mecz {xgdpm:+.2f} (Summary {xn}/{v['m']})"
            items.append(cand(pid,score,reason,
                matches=int(v["m"]),wins=int(v["w"]),draws=int(v["d"]),losses=int(v["l"]),
                titles=int(v["titles"]),finals=int(v["finals"]),win_pct=round(wp,1),
                gf=int(v["gf"]),ga=int(v["ga"]),gd=int(v["gf"]-v["ga"]),
                clutch_wins=int(v["clutch_w"]),clutch_matches=int(v["clutch_m"]),clutch_pct=round(cl,1),
        ...
        for pid,v in ps.items():
            ...
            if xn>0:
                ...
                score+=max(-1.5,min(1.5,2.0-xgapm))*4.0*_summary_strength(pid,xn)
                reason+=f" • xGA {xgapm:.2f}/m (Summary {xn}/{v['m']})"
            items.append(cand(pid,score,reason,
                matches=int(v["m"]),ga=int(v["ga"]),ga_per_match=round(ga_pm,2),
        ...
        for pid,v in ps.items():
            ...
            items.append(cand(pid,(lpts-epts)*30+(lgd-egd)*10,f"punkty/mecz {epts:.2f} → {lpts:.2f} • bilans bramek/mecz {egd:+.2f} → {lgd:+.2f}",
                matches=int(v["m"]),win_pct=pc(pid,v),early_points_per_match=round(epts,2),late_points_per_match=round(lpts,2),
                early_gd_per_match=round(egd,2),late_gd_per_match=round(lgd,2)))
        ...
        for pid,v in ps.items():
            ...
            reason=f"{avg_goals:.2f} gola/mecz w jego spotkaniach • {spectacular}/{len(vals)} bardzo widowiskowych • {v['spectacle_pens']} mecz(e) z karnymi"
            sp=summary_ps.get(pid) or {};sn=int(sp.get("spectacle_n") or 0)
            if sn>0:
                sbavg=float(sp.get("spectacle_bonus_sum") or 0.0)/sn
                score+=sbavg*80*_summary_strength(pid,sn)
                xgn=int(sp.get("xg_n") or 0);shn=int(sp.get("shots_n") or 0)
                extras=[]
                if xgn>0:extras.append(f"łączne xG {(float(sp.get('xg_for') or 0)+float(sp.get('xg_against') or 0))/xgn:.2f}/m")
                if shn>0:extras.append(f"strzały {(float(sp.get('shots_for') or 0)+float(sp.get('shots_against') or 0))/shn:.1f}/m")
                if extras:reason+=" • "+" • ".join(extras)+f" (Summary {sn}/{len(vals)})"
            items.append(cand(pid,score,reason,
                matches=len(vals),avg_goals=round(avg_goals,2),spectacular_matches=int(spectacular),
                penalty_matches=int(v["spectacle_pens"]),summary_matches=int(sn),
                summary_total_xg_per_match=(round((float(sp.get("xg_for") or 0)+float(sp.get("xg_against") or 0))/int(sp.get("xg_n") or 1),2) if int(sp.get("xg_n") or 0)>0 else None),
                summary_total_shots_per_match=(round((float(sp.get("shots_for") or 0)+float(sp.get("shots_against") or 0))/int(sp.get("shots_n") or 1),1) if int(sp.get("shots_n") or 0)>0 else None)
            ))
        add("spectacle","🎆 Najbardziej Widowiskowy Gracz","Jeśli gra, zwykle coś się dzieje. Gole, końcówki i karne pozostają rdzeniem; Summary delikatnie premiuje wysokie xG i liczbę strzałów. Minimum 5 meczów.",items)
        items=[]
        for pid,v in ps.items():
        ...
        for pid,matches_n in detailed_matches_by_player.items():
            ...
            sp=summary_ps.get(pid) or {};fn=int(sp.get("fouls_n") or 0);foul_pm=(float(sp.get("fouls") or 0)/fn if fn>0 else None)
            fair_metric=ppm
            reason=f"{d['points']} pkt dyscypliny w {matches_n} meczach • {ppm:.2f} pkt/mecz • 🟨 {d['yellow']} • 🟥 {d['red']}"
            if foul_pm is not None:
                # Fouls are deliberately much weaker than actual cards.
                fair_metric+=min(foul_pm,8.0)*.03*_summary_strength(pid,fn)
                reason+=f" • {foul_pm:.2f} faulu/mecz (Summary {fn}/{matches_n})"
            items.append({
                "id":str(pid),"name":name_by.get(str(pid),"?"),
                "score":round(100-fair_metric*20,4),"_sort":(-fair_metric,int(matches_n),-int(d["points"])),
                "reason":reason,"matches":int(matches_n),"discipline_points":int(d["points"]),
                "discipline_per_match":round(ppm,2),"yellow":int(d["yellow"]),"red":int(d["red"]),
                "fouls_total":(int(round(float(sp.get("fouls") or 0))) if fn>0 else None),"fouls_matches":int(fn),
                "fouls_per_match":(round(foul_pm,2) if foul_pm is not None else None)
            })
        add("fair_play","😇 Fair Play","Kartki są najważniejsze; Summary dodaje mały komponent fauli, jeśli jest dostępny. Minimum 5 meczów ze szczegółowym przebiegiem.",items)
        ...
        for (pid,sn),goals in scorer_by_player.items():
            ...
            items.append({
                ...
                "goals":int(goals),"matches":int(team_matches),"hattricks":int(hattricks),
                "footballer":scorer,"fifa_night_player":player
            })
        ...
        for pid,v in finance.items():
            bal=v["won"]-v["paid"];fin_items.append(cand(pid,bal/100,f"bilans {(bal/100):+.2f} zł • wygrane {v['won']/100:.2f} zł • wpłaty {v['paid']/100:.2f} zł",
                balance_pln=round(bal/100,2),won_pln=round(v["won"]/100,2),paid_pln=round(v["paid"]/100,2)))
        ...
        for (a,b),v in pair.items():
            ...
            # -> more valuable direct wins -> direct-H2H fair play -> official draw.
            winner=None;loser=None;decider="remis"
            if int(v["aw"])!=int(v["bw"]):
                winner=a if int(v["aw"])>int(v["bw"]) else b;decider="zwycięstwa H2H"
            elif int(v["goals_a"])!=int(v["goals_b"]):
                winner=a if int(v["goals_a"])>int(v["goals_b"]) else b;decider="bilans bramek H2H"
            else:
                ta=tuple(int(v["stage_wins_a"].get(w,0)) for w in (4,3,2,0))
                tb=tuple(int(v["stage_wins_b"].get(w,0)) for w in (4,3,2,0))
                if ta!=tb:
                    winner=a if ta>tb else b;decider="ważniejsze bezpośrednie mecze"
                elif int(v.get("fairplay_a") or 0)!=int(v.get("fairplay_b") or 0):
                    winner=a if int(v.get("fairplay_a") or 0)<int(v.get("fairplay_b") or 0) else b;decider="fair play H2H"
            if winner:loser=b if winner==a else a

            detail=[]
            if int(v.get("finals") or 0):detail.append(f"{int(v['finals'])} finał(y)")
            if int(v.get("semifinals") or 0):detail.append(f"{int(v['semifinals'])} półfinał(y)")
            if int(v.get("knockouts") or 0):detail.append(f"{int(v['knockouts'])} inne ważne")
            if int(v.get("penalty_matches") or 0):detail.append(f"{int(v['penalty_matches'])} po karnych")
            reason=f"{v['n']} meczów • {na} {v['aw']}:{v['bw']} {nb} • bramki {v['goals_a']}:{v['goals_b']}"
            if detail:reason+=" • "+" • ".join(detail)
            reason+=f" • zwycięzca rywalizacji: {name_by.get(winner,'remis') if winner else 'remis'}"
            rivalry.append({
                "id":f"{a}|{b}","name":f"{na} vs {nb}","score":round(score,2),"reason":reason,
                "participant_ids":[a,b],"participant_names":[na,nb],"matches":int(v["n"]),
                "wins_a":int(v["aw"]),"wins_b":int(v["bw"]),"draws":int(v["d"]),
        ...
        _match_order={(str(m["tournament_id"]),int(m["match_no"])):i for i,m in enumerate(matches)}
        scorer_items=[]
        for sn,goals in scorer_totals.items():
            if goals<5:continue
            cumulative=0;progress=[0]
            for mk,g in sorted((scorer_goals_by_match.get(sn) or {}).items(),key=lambda kv:_match_order.get(kv[0],10**9)):
                cumulative+=int(g or 0);progress.append(cumulative)
            scorer_items.append({
