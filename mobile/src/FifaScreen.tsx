...
const WHEEL_COLORS=['#2563EB','#DB2777','#0891B2','#EA580C','#16A34A','#7C3AED','#CA8A04','#DC2626'];
const wheelShort=(value:any)=>{const s=String(value||'');const wc=s.match(/Dowolna drużyna(?: #(\d+))?/i);if(wc)return (wc[1]?`WC ${wc[1]}`:'WILD CARD');return s.replace('Bayern Monachium','BAYERN').replace('FC Barcelona','BARCA').replace('Manchester City','MAN CITY').slice(0,11).toUpperCase()};
...
function TeamWheelReveal({event,pool,onDone}:{event:any;pool:string[];onDone:()=>void}){
 ...
 const result=String(event?.wildcard?'🃏 Wild Card':(event?.team||event?.wheel_team||''));
 const labels=(pool||[]);
 ...
 const endDeg=2880+((360-angle)%360);
 const radius=107;
 const sectorBase=labels.length?Math.min(radius*2,2*radius*Math.tan(Math.PI/Math.max(labels.length,3))):0;
 useEffect(()=>{
  spin.setValue(0);
  Animated.timing(spin,{toValue:1,duration:10000,useNativeDriver:true}).start();
  const timer=setTimeout(()=>doneRef.current(),12200);return()=>clearTimeout(timer);
 ...
 return <View style={st.wheelReveal}><Text style={st.smallLabel}>LOSOWANIE DRUŻYNY</Text><Text style={st.bigName}>Teraz losujemy dla {event?.name||'gracza'}</Text><View style={st.wheelShell}><View style={st.wheelPointer}/><Animated.View style={[st.wheelRing,{transform:[{rotate}]}]}><View style={st.wheelSegments}>{labels.map((x:string,i:number)=>{const a=(360/Math.max(labels.length,1))*i;return <View key={`seg-${x}-${i}`} style={[st.wheelSectorWrap,{transform:[{rotate:`${a}deg`}]}]}><View style={[st.wheelSector,{left:radius-sectorBase/2,borderLeftWidth:sectorBase/2,borderRightWidth:sectorBase/2,borderTopWidth:radius,borderTopColor:WHEEL_COLORS[i%WHEEL_COLORS.length]}]}/></View>})}</View>{labels.map((x:string,i:number)=>{const a=(360/Math.max(labels.length,1))*i;return <View key={`${x}-${i}`} style={[st.wheelLabelWrap,{transform:[{rotate:`${a}deg`},{translateY:-92},{rotate:`-${a}deg`}]}]}><Text style={st.wheelLabel}>{wheelShort(x)}</Text></View>})}<View style={st.wheelHub}><Text style={st.wheelHubText}>FC</Text></View></Animated.View></View><RevealMotion revealKey={`${event?.player_id}-${result}`} delay={9850}><Text style={st.wheelResult}>{result}</Text></RevealMotion></View>;
}
...
function MatchCard({m,current=false,onDetails}:{m:Match;current?:boolean;onDetails?:()=>void}){
 ...
 return <Pressable onPress={onDetails} disabled={!onDetails} style={[st.matchCard,current&&st.current,played&&st.played]}>
  ...
  <View style={st.matchNames}><View style={{flex:1}}><Text style={st.name}>{m.home_name||m.home_source_display||'Czeka na rozstrzygnięcie'}</Text><Text style={st.team}>{m.home_team||(m.home_name?'—':'ścieżka drabinki')}</Text></View><Text style={st.vs}>VS</Text><View style={{flex:1}}><Text style={st.name}>{m.away_name||m.away_source_display||'Czeka na rozstrzygnięcie'}</Text><Text style={st.team}>{m.away_team||(m.away_name?'—':'ścieżka drabinki')}</Text></View></View>
 </Pressable>
}
...
function StartWizard({options,controller,refresh,setBusy}:{options:any;controller:boolean;refresh:()=>Promise<void>;setBusy:(v:boolean)=>void}){
 ...
 const [cash,setCash]=useState<boolean[]>(Array(10).fill(true));
 ...
 const toggleMorePlayers=()=>{if(morePlayers){if(['8','9','10'].includes(variant))setVariant('7');setMorePlayers(false)}else setMorePlayers(true)};
 const create=async()=>{setError('');setBusy(true);try{
  ...
  if(variant==='1v1'){
   ...
   await api.createDuel({player1:names[0],player2:names[1],team1:teams[0],team2:teams[1],game_version:gameVersion,stake_per_player:stakeValue,cash1:cash[0],cash2:cash[1]});
  }else{
   if(names.slice(0,count).some(x=>!x.trim()))throw new Error('Wpisz wszystkich graczy.');
   await api.createTournament({player_count:count,format_key:format,game_version:gameVersion,is_test:controller?isTest:true,stake_per_player:stakeValue,players:names.slice(0,count).map((name,i)=>({name,cash:cash[i]}))});
  }
  await refresh();
 }catch(e:any){setError(e.message)}finally{setBusy(false)}};
 const teamSuggestions=[...((options?.team_pools?.[gameVersion]?.['5'])||options?.fixed_teams||[]),...((options?.wildcard_suggestions_by_version?.[gameVersion])||options?.wildcard_suggestions||[]),...(options?.real_helper_team?[options.real_helper_team]:[])];
 return (
  <ScrollView contentContainerStyle={st.pad} keyboardShouldPersistTaps="handled">
   <Card style={st.hero}>
    <Pill text="NOWY FIFA NIGHT" tone="green"/>
    <Text style={st.heroTitle}>Wybierz wariant. Resztą zajmiemy się po drodze.</Text>
    <Muted>Pełny oficjalny turniej 3–10 wymaga sterowania. Testy i oficjalne 1 vs 1 możesz uruchomić bez niego.</Muted>
   </Card>
   <SectionTitle>Wariant</SectionTitle>
   <HScroll>{['1v1','3','4','5','6','7'].map(x=><Chip key={x} label={x==='1v1'?'⚔️ 1 VS 1':`${x} graczy`} active={variant===x} onPress={()=>setVariant(x)}/>)}</HScroll>
   <Btn title={morePlayers?'− MNIEJ ZAWODNIKÓW':'+ WIĘCEJ ZAWODNIKÓW'} tone="secondary" onPress={toggleMorePlayers}/>
   {morePlayers ? <HScroll>{['8','9','10'].map(x=><Chip key={x} label={`${x} graczy`} active={variant===x} onPress={()=>setVariant(x)}/>)}</HScroll> : null}
   <SectionTitle>Wersja gry</SectionTitle>
   <HScroll>{(options?.game_versions||['FC26','FC27']).map((v:string)=><Chip key={v} label={v} active={gameVersion===v} onPress={()=>setGameVersion(v)}/>)}</HScroll>
   {variant!=='1v1' ? <>
    <SectionTitle>Format</SectionTitle>
    <View style={{gap:8}}>{(options?.formats?.[String(count)]||[]).map((f:any)=><Pressable key={f.key} onPress={()=>setFormat(f.key)} style={[st.option,format===f.key&&st.optionActive]}><Text style={st.optionTitle}>{f.label}</Text><Text style={st.optionSub}>{f.matches}</Text></Pressable>)}</View>
   </> : null}
   <SectionTitle>Skład</SectionTitle>
   {Array.from({length:count},(_,i)=><PlayerEntry key={i} idx={i} value={names[i]||''} cash={cash[i]??true} onName={(v:string)=>setName(i,v)} onCash={v=>setCashFlag(i,v)} suggestions={options?.players||[]}/>) }
   ...
   <Btn title="🎮 UTWÓRZ ROZGRYWKĘ" onPress={create}/>
  </ScrollView>
 )

}

function StructurePairPreview({rows,revealKey}:{rows:any[];revealKey:string}){
 const side=(src:any,pos:number)=>{if(!src)return null;if(src.kind==='player'){const delay=900+Number(src.reveal_order||0)*4500;return <RevealMotion revealKey={`${revealKey}-${src.slot}-${src.player_id}`} delay={delay}><View style={st.structurePlayer}><Text style={st.smallLabel}>{pos===0?'1. LOS':'2. LOS'}</Text><Text style={st.structurePlayerName}>{src.name||'?'}</Text></View></RevealMotion>}return <View style={st.structureRef}><Text style={st.smallLabel}>ZALEŻNOŚĆ</Text><Text style={st.structureRefName}>{src.name||src.label||'?'}</Text></View>};
 return <View style={{gap:10}}>{rows.map((x:any,i:number)=><View key={`${revealKey}-${x.match_no||'bye'}-${i}`} style={[st.structureMatch,x.stage==='PLAY_IN'&&st.structurePlayIn]}><Pill text={`${x.match_no?`M${x.match_no} • `:''}${x.stage_label||x.stage||'MECZ'}`} tone={x.stage==='PLAY_IN'?'amber':'blue'}/>{x.away?<View style={st.structureDuel}><View style={{flex:1}}>{side(x.home,0)}</View><Text style={st.structureVs}>VS</Text><View style={{flex:1}}>{side(x.away,1)}</View></View>:<View style={st.structureDuel}><View style={{flex:1}}>{side(x.home,0)}</View><View style={[st.structureRef,{flex:1,backgroundColor:'#26330f',borderColor:'#64791d'}]}><Text style={st.structureRefName}>🍀 WOLNY LOS</Text></View></View>}</View>)}</View>
}
...
function SetupStage({t,controller,refresh,setBusy}:{t:LiveTournament;controller:boolean;refresh:()=>Promise<void>;setBusy:(v:boolean)=>void}){
 const [setup,setSetup]=useState<SetupResponse|null>(null); const [error,setError]=useState(''); const [wild,setWild]=useState(''); const [pick,setPick]=useState(''); const [wheelReveal,setWheelReveal]=useState<any>(null); const [draftRevealReady,setDraftRevealReady]=useState(true); const [structureRevealReady,setStructureRevealReady]=useState(true);
 ...
 const structurePreview=setup.structure_preview||[];
 ...
 const structureSteps=structurePreview.length?1+Math.max(...structurePreview.flatMap((x:any)=>[x.home?.reveal_order,x.away?.reveal_order].filter((v:any)=>v!=null)).map((v:any)=>Number(v)),0):groups().length;
 return <ScrollView contentContainerStyle={st.pad}><Card style={st.hero}><View style={st.row}><Pill text={t.is_test?'🧪 TEST':'🏆 OFICJALNY'} tone={t.is_test?'amber':'green'}/><Pill text={String(t.game_version||'FC26')} tone="blue"/><Pill text={`${t.player_count} GRACZY`} tone="blue"/></View><Text style={st.heroTitle}>{t.format_label}</Text><Muted>{phase==='draft_order'?'Etap 1/3 • kolejność draftu':phase==='team_draft'?'Etap 2/3 • wybór drużyn':phase==='team_draw'?'Etap 1/2 • koło drużyn':'Losowanie struktury turnieju'}</Muted></Card>
  {!editable?<Card><Muted>Ten oficjalny etap wymaga sterowania na urządzeniu.</Muted></Card>:null}
  {phase==='draft_order'?<Card><Text style={st.cardTitle}>🎱 Kolejność wyboru drużyn</Text>{extra.draft_order_revealed?<View style={{gap:6}}>{players.map((p:any,i)=><RevealMotion key={p.player_id} revealKey={`draft-${extra.draft_redraw_count||0}-${p.player_id}`} delay={900+i*4500}><View style={st.drawRow}><Text style={st.drawNo}>{i+1}</Text><Text style={st.bigName}>{p.name}</Text></View></RevealMotion>)}</View>:<Muted>Kolejność jest jeszcze zakryta.</Muted>}<Btn title="🎱 LOSUJ KOLEJNOŚĆ" disabled={!editable||extra.draft_order_revealed} onPress={()=>{setDraftRevealReady(false);act(()=>api.draftReveal(t.id),()=>setTimeout(()=>setDraftRevealReady(true),2200+Math.max(players.length-1,0)*4500))}}/>{extra.draft_order_revealed?<><View style={st.two}><View style={{flex:1}}><Btn title={draftRevealReady?"💸 LOSUJ PONOWNIE":"⏳ ODKRYWAM…"} tone="secondary" disabled={!editable||!draftRevealReady} onPress={()=>{setDraftRevealReady(false);act(()=>api.draftReroll(t.id),()=>setTimeout(()=>setDraftRevealReady(true),2200+Math.max(players.length-1,0)*4500))}}/></View><View style={{flex:1}}><Btn title={draftRevealReady?"✅ ZATWIERDŹ":"⏳ ODKRYWAM…"} disabled={!editable||!draftRevealReady} onPress={()=>act(()=>api.draftConfirm(t.id))}/></View></View></>:null}</Card>:null}
  {phase==='team_draft'?<Card><Text style={st.cardTitle}>⚽ Draft drużyn</Text>{players.filter((p:any)=>p.team_revealed).map((p:any)=><Text style={st.line} key={p.player_id}>✓ {p.name}: <Text style={st.green}>{p.team}</Text></Text>)}{waiting?<><Text style={st.bigName}>Teraz wybiera: {waiting.name}</Text><HScroll>{(setup.draft_available||[]).map(x=><Chip key={x} label={x} active={pick===x} onPress={()=>setPick(x)}/>)}</HScroll>{pick.includes('Wild')?<><Input value={wild} onChangeText={setWild} placeholder="Wpisz klub Wild Card"/><HScroll>{(setup.wildcard_suggestions||[]).slice(0,8).map(x=><Chip key={x} label={x} onPress={()=>setWild(x)}/>)}</HScroll></>:null}<Btn title="✅ WYBIERZ DRUŻYNĘ" disabled={!editable||!pick} onPress={()=>act(()=>api.draftPick(t.id,{player_id:waiting.player_id,slot:pick,wildcard_name:wild}))}/>{!(setup.meta?.extra?.cash_player_ids||[]).includes(waiting.player_id)?<Btn title="🤍 REAL MADRYT — POMOC" disabled={!editable} onPress={()=>act(()=>api.realHelper(t.id,waiting.player_id))}/>:null}</>:<Btn title="🎲 PRZEJDŹ DO LOSOWANIA TURNIEJU" disabled={!editable} onPress={()=>act(()=>api.structureStart(t.id))}/>}</Card>:null}
  {phase==='team_draw'?<Card><Text style={st.cardTitle}>🎡 Koło drużyn</Text>{wheelReveal?<TeamWheelReveal event={wheelReveal} pool={wheelReveal?.pool||setup.meta?.team_pool||setup.team_pool||[]} onDone={()=>setWheelReveal(null)}/>:null}{!wheelReveal?players.filter((p:any)=>p.team_revealed).map((p:any,i:number)=><RevealMotion key={`${p.player_id}-${p.team}`} revealKey={`${p.player_id}-${p.team}`} delay={Math.min(i*55,220)}><Text style={st.line}>✓ {p.name}: <Text style={st.green}>{p.team}</Text></Text></RevealMotion>):null}{pending&&!wheelReveal?<RevealMotion revealKey={`wild-${pending.player_id}`}><View style={{gap:8}}><Text style={st.bigName}>🃏 Wild Card dla {pending.name}</Text><Input value={wild} onChangeText={setWild} placeholder="Wpisz drużynę"/><HScroll>{(setup.wildcard_suggestions||[]).slice(0,8).map(x=><Chip key={x} label={x} onPress={()=>setWild(x)}/>)}</HScroll><Btn title="✅ ZATWIERDŹ WILD CARD" disabled={!editable||!wild.trim()} onPress={()=>act(()=>api.wildcard(t.id,{player_id:pending.player_id,team_name:wild}))}/></View></RevealMotion>:waiting&&!wheelReveal?<><Btn title="🎡 ZAKRĘĆ KOŁEM" disabled={!editable||!!wheelReveal} onPress={()=>act(()=>api.teamReveal(t.id),(r:any)=>setWheelReveal(r?.revealed||null))}/>{!(setup.meta?.extra?.cash_player_ids||[]).includes(waiting.player_id)?<Btn title="🤍 REAL MADRYT — POMOC" tone="secondary" disabled={!editable} onPress={()=>act(()=>api.realHelper(t.id,waiting.player_id))}/>:null}</>:!waiting&&!wheelReveal?<Btn title="🎲 PRZEJDŹ DO LOSOWANIA TURNIEJU" disabled={!editable} onPress={()=>act(()=>api.structureStart(t.id))}/>:null}</Card>:null}
  {phase==='structure_draw'?<Card><Text style={st.cardTitle}>🎲 Struktura turnieju</Text>{setup.meta?.draw_revealed?<><Muted>{structurePreview.length?'Oficjalne pary pierwszej fazy:':'Oficjalne losowanie FIFA Night:'}</Muted>{structurePreview.length?<StructurePairPreview rows={structurePreview} revealKey={`structure-${setup.meta?.redraw_count||0}`}/>:<View style={{gap:7}}>{groups().map((x,i)=><RevealMotion key={`${setup.meta?.redraw_count||0}-${x}`} revealKey={`${setup.meta?.redraw_count||0}-${x}`} delay={1000+i*4500}><View style={st.structureRow}><Text style={st.line}>{x}</Text></View></RevealMotion>)}</View>}</>:<Muted>Pary / grupy są jeszcze zakryte.</Muted>}{!setup.meta?.draw_revealed?<Btn title="🎲 LOSUJ" disabled={!editable} onPress={()=>{setStructureRevealReady(false);act(()=>api.structureReveal(t.id),()=>setTimeout(()=>setStructureRevealReady(true),2400+Math.max(structureSteps-1,0)*4500))}}/>:<View style={st.two}><View style={{flex:1}}><Btn title={structureRevealReady?"🔄 PONÓW":"⏳ ODKRYWAM…"} tone="secondary" disabled={!editable||!structureRevealReady} onPress={()=>{setStructureRevealReady(false);act(()=>api.structureReroll(t.id),()=>setTimeout(()=>setStructureRevealReady(true),2400+Math.max(structureSteps-1,0)*4500))}}/></View><View style={{flex:1}}><Btn title={structureRevealReady?"✅ START":"⏳ ODKRYWAM…"} disabled={!editable||!structureRevealReady} onPress={()=>act(()=>api.structureConfirm(t.id))}/></View></View>}</Card>:null}
  {error?<ErrorBox message={error}/>:null}
...
function SpecialDraw({t,canEdit,refresh}:{t:LiveTournament;canEdit:boolean;refresh:()=>Promise<void>}){
 ...
 const bigBye=d?.bye_player_id?(d?.bye_candidates||[]).find((x:any)=>String(x.player_id)===String(d.bye_player_id)):null;
 const revealKey=d?`${d.kind}-${d.selected_lucky?.player_id||d.selected_lucky?.name||''}-${d.selected?.player_id||d.selected?.name||''}-${bigBye?.player_id||''}-${(d.pairs||[]).map((p:any)=>`${p.match_no||p.key}:${p.home_name}:${p.away_name}`).join('|')}`:'none';
 useEffect(()=>{if(!d?.selected){setReadyAck(false);return}if(d?.is_random_draw===false){setReadyAck(true);return}const hasLucky=!!(d?.selected_lucky||((d?.kind==='double7_lb_bye'&&d?.selected)?d.selected:null)||bigBye);const pairCount=(d.pairs||[]).length;const wait=Math.max(7000,4400+Math.max(pairCount-1,0)*4500+(hasLucky?2800:0));const timer=setTimeout(()=>setReadyAck(true),wait);return()=>clearTimeout(timer)},[revealKey,d?.selected,(d?.pairs||[]).length]);
 if(!d)return null;
 ...
 const randomDraw=d.is_random_draw!==false;
 const title=d.kind==='group_playoffs'?'🎯 FAZA PUCHAROWA':d.kind==='double7_combined'?'🎟️ WINNERS + SZCZĘŚLIWY LOS':d.kind==='double5_opponent'?'🎟️ LOSOWANIE RYWALA':d.kind==='double7_lb_bye'?'🍀 SZCZĘŚLIWY LOS':'🎲 LOSOWANIE DRABINKI';
 const subtitle=d.kind==='double7_combined'?'Najpierw pary Winners, potem jedna osoba dostaje wolny los.':d.kind==='group_playoffs'?'Pary wynikają z zasad formatu — np. 1A–2B i 1B–2A. Nie losujemy ich ponownie.':'Jedno losowanie może zmienić całą drogę do finału.';
 const lucky=d.selected_lucky||((d.kind==='double7_lb_bye'&&d.selected)?d.selected:null)||bigBye;
 const pairRows=(d.pairs||[]).map((p:any,i:number)=>{const badge=`${p.match_no?`M${p.match_no}`:(p.key||'PARA')}${p.stage_label?` • ${p.stage_label}`:p.stage?` • ${p.stage}`:''}`;if(!randomDraw)return <View key={`${revealKey}-${p.match_no||p.key}`} style={st.specialPair}><Text style={st.smallLabel}>{badge}</Text><View style={st.specialPairDuel}><View style={{flex:1}}><Text style={st.specialPairName}>{p.home_name}</Text></View><Text style={st.vs}>VS</Text><View style={{flex:1}}><Text style={st.specialPairName}>{p.away_name}</Text></View></View></View>;const base=900+i*4500;return <View key={`${revealKey}-${p.match_no||p.key}`} style={st.specialPair}><Text style={st.smallLabel}>{badge}</Text><View style={st.specialPairDuel}><View style={{flex:1}}><RevealMotion revealKey={`${revealKey}-${p.match_no||p.key}-h`} delay={base}><View style={st.specialPairSide}><Text style={st.smallLabel}>1. LOS</Text><Text style={st.specialPairName}>{p.home_name}</Text></View></RevealMotion></View><Text style={st.vs}>VS</Text><View style={{flex:1}}><RevealMotion revealKey={`${revealKey}-${p.match_no||p.key}-a`} delay={base+2250}><View style={st.specialPairSide}><Text style={st.smallLabel}>2. LOS</Text><Text style={st.specialPairName}>{p.away_name}</Text></View></RevealMotion></View></View></View>});
 return <Card style={st.special}><Pill text={randomDraw?'LOSOWANIE W TRAKCIE':'PARY USTALONE'} tone={randomDraw?'amber':'green'}/><Text style={st.cardTitle}>{title}</Text><Muted>{subtitle}</Muted>{d.selected?<View style={{gap:8}}>{pairRows}{lucky?<RevealMotion revealKey={`${revealKey}-lucky`} delay={1200+(d.pairs||[]).length*4500}><View style={st.luckyReveal}><Text style={{fontSize:30}}>🍀</Text><View style={{flex:1}}><Text style={st.smallLabel}>SZCZĘŚLIWY LOS</Text><Text style={st.bigName}>{lucky.name}</Text></View></View></RevealMotion>:null}</View>:<View style={st.drawWaiting}><Text style={{fontSize:38}}>🎲</Text><Muted center>Wynik jest jeszcze zakryty.</Muted></View>}{d.selected?<Btn title={randomDraw?(readyAck?'✅ ZATWIERDŹ I GRAJ DALEJ':'⏳ ODKRYWAM…'):'✅ ZACZYNAMY FAZĘ PUCHAROWĄ'} disabled={!canEdit||(randomDraw&&!readyAck)} onPress={ack}/>:<Btn title="🎲 ODKRYJ LOSOWANIE" disabled={!canEdit} onPress={reveal}/>} {error?<ErrorBox message={error}/>:null}</Card>;
}
