...
function SetupStage({t,controller,refresh,setBusy}:{t:LiveTournament;controller:boolean;refresh:()=>Promise<void>;setBusy:(v:boolean)=>void}){
 ...
 return <ScrollView contentContainerStyle={st.pad}><Card style={st.hero}><View style={st.row}><Pill text={t.is_test?'🧪 TEST':'🏆 OFICJALNY'} tone={t.is_test?'amber':'green'}/><Pill text={String(t.game_version||'FC26')} tone="blue"/><Pill text={`${t.player_count} GRACZY`} tone="blue"/></View><Text style={st.heroTitle}>{t.format_label}</Text><Muted>{phase==='draft_order'?'Etap 1/3 • kolejność draftu':phase==='team_draft'?'Etap 2/3 • wybór drużyn':phase==='team_draw'?'Etap 1/2 • koło drużyn':'Losowanie struktury turnieju'}</Muted></Card>
  ...
  {phase==='structure_draw'?<Card><Text style={st.cardTitle}>🎲 Struktura turnieju</Text>{setup.meta?.draw_revealed?<><Muted>Oficjalne losowanie FIFA Night:</Muted><View style={{gap:7}}>{groups().map((x,i)=><RevealMotion key={`${setup.meta?.redraw_count||0}-${x}`} revealKey={`${setup.meta?.redraw_count||0}-${x}`} delay={1000+i*4500}><View style={st.structureRow}><Text style={st.line}>{x}</Text></View></RevealMotion>)}</View></>:<Muted>Pary / grupy są jeszcze zakryte.</Muted>}{!setup.meta?.draw_revealed?<Btn title="🎲 LOSUJ" disabled={!editable} onPress={()=>{setStructureRevealReady(false);act(()=>api.structureReveal(t.id),()=>setTimeout(()=>setStructureRevealReady(true),2400+Math.max(groups().length-1,0)*4500))}}/>:<View style={st.two}><View style={{flex:1}}><Btn title={structureRevealReady?"🔄 PONÓW":"⏳ ODKRYWAM…"} tone="secondary" disabled={!editable||!structureRevealReady} onPress={()=>{setStructureRevealReady(false);act(()=>api.structureReroll(t.id),()=>setTimeout(()=>setStructureRevealReady(true),2400+Math.max(groups().length-1,0)*4500))}}/></View><View style={{flex:1}}><Btn title={structureRevealReady?"✅ START":"⏳ ODKRYWAM…"} disabled={!editable||!structureRevealReady} onPress={()=>act(()=>api.structureConfirm(t.id))}/></View></View>}</Card>:null}
  {error?<ErrorBox message={error}/>:null}
...
function SpecialDraw({t,canEdit,refresh}:{t:LiveTournament;canEdit:boolean;refresh:()=>Promise<void>}){
 ...
 useEffect(()=>{if(!d?.selected){setReadyAck(false);return}if(d?.is_random_draw===false){setReadyAck(true);return}const hasLucky=!!(d?.selected_lucky||((d?.kind==='double7_lb_bye'&&d?.selected)?d.selected:null)||bigBye);const steps=(d.pairs||[]).length+(hasLucky?1:0);const wait=Math.max(5200,2200+Math.max(steps-1,0)*4500);const timer=setTimeout(()=>setReadyAck(true),wait);return()=>clearTimeout(timer)},[revealKey,d?.selected,(d?.pairs||[]).length]);
 if(!d)return null;
 ...
 const pairRows=(d.pairs||[]).map((p:any,i:number)=>randomDraw?<RevealMotion key={`${revealKey}-${p.match_no}`} revealKey={`${revealKey}-${p.match_no}`} delay={1000+i*4500}><View style={st.specialPair}><Text style={st.smallLabel}>{p.match_no?`M${p.match_no}`:(p.key||'PARA')}</Text><Text style={st.specialPairText}>{p.home_name} <Text style={st.vs}>VS</Text> {p.away_name}</Text></View></RevealMotion>:<View key={`${revealKey}-${p.match_no}`} style={st.specialPair}><Text style={st.smallLabel}>{p.match_no?`M${p.match_no}`:(p.key||'PARA')}</Text><Text style={st.specialPairText}>{p.home_name} <Text style={st.vs}>VS</Text> {p.away_name}</Text></View>);
