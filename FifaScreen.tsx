...
function ScoreModal({t,matchNo,onClose,refresh,canEdit}:{t:LiveTournament;matchNo:number|null;onClose:()=>void;refresh:()=>Promise<void>;canEdit:boolean}){
 ...
 const [images,setImages]=useState<string[]>([]);const [events,setEvents]=useState<any[]|null>(null);const [scanInfo,setScanInfo]=useState<any>(null);
 ...
 const pickImage=async(camera:boolean)=>{setErr('');try{if(camera){const perm=await ImagePicker.requestCameraPermissionsAsync();if(!perm.granted)throw new Error('Brak zgody na aparat.');const r=await ImagePicker.launchCameraAsync({mediaTypes:['images'] as any,quality:.9});const uri=!r.canceled?r.assets?.[0]?.uri:undefined;if(uri)setImages(a=>[...a,uri].slice(0,6));}else{const r=await ImagePicker.launchImageLibraryAsync({mediaTypes:['images'] as any,allowsMultipleSelection:true,selectionLimit:6,quality:.9});if(!r.canceled)setImages(a=>[...a,...r.assets.map((x:any)=>x.uri)].slice(0,6));}}catch(e:any){setErr(e.message)}};
 ...
 return <Modal visible={visible} animationType="slide" onRequestClose={onClose}><View style={st.modal}><View style={st.modalHead}><View><Text style={st.kicker}>MECZ {m.match_no}</Text><Text style={st.modalTitle}>{m.home_name} — {m.away_name}</Text></View><Pressable onPress={onClose}><Text style={st.close}>×</Text></Pressable></View><ScrollView contentContainerStyle={st.pad} keyboardShouldPersistTaps="handled">
  ...
  {canEdit?<Card><Text style={st.cardTitle}>📷 Odczytaj zdarzenia z EA FC</Text><Muted>Zrób zdjęcie ekranu Events. Jeśli lista jest dłuższa, dodaj kolejne. Najpierw zobaczysz odczyt i możesz go poprawić — zapis nastąpi dopiero po zatwierdzeniu wyniku.</Muted><View style={st.two}><View style={{flex:1}}><Btn title="📷 APARAT" tone="secondary" onPress={()=>pickImage(true)}/></View><View style={{flex:1}}><Btn title="🖼️ GALERIA" tone="secondary" onPress={()=>pickImage(false)}/></View></View>{images.length?<><ScrollView horizontal contentContainerStyle={{gap:8}}>{images.map((u,i)=><View key={`${u}-${i}`} style={{gap:4}}><Image source={{uri:u}} style={st.thumb}/><Pressable onPress={()=>setImages(a=>a.filter((_,j)=>j!==i))}><Text style={st.red}>Usuń</Text></Pressable></View>)}</ScrollView><Btn title={saving?'ODCZYTUJĘ…':'🔎 ODCZYTAJ ZDJĘCIA'} disabled={saving} onPress={scan}/></>:null}
