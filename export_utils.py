...
BG="#0a1020"; PANEL="#111a2b"; PANEL2="#162238"; BORDER="#2b3a55"
TEXT="#f8fafc"; MUTED="#a8b3c7"; GOLD="#facc15"; CYAN="#38bdf8"; GREEN="#22c55e"; PINK="#fb7185"; PURPLE="#a78bfa"
_FONT_CACHE={}
...
def _font(size:int,bold:bool=False):
    ...
    paths=[]
    for n in names:
        paths += [n, f"/usr/share/fonts/truetype/dejavu/{n}", f"/usr/share/fonts/dejavu/{n}"]
    for path in paths:
        try:
            f=ImageFont.truetype(path,size);_FONT_CACHE[key]=f;return f
...
def _date(value):
    ...
    except Exception:return str(value or "")[:10] or "—"


def _team_for(bundle,name):
    for p in bundle.get("players",[]):
        if str(p.get("name"))==str(name):return str(p.get("team") or "—")
    return "—"
...
def _champion_record(bundle,champ):
    ...
    for m in bundle.get("matches",[]):
        if m.get("home_score") is None:continue
        h,a=m.get("home_name"),m.get("away_name");hs,aw=int(m.get("home_score") or 0),int(m.get("away_score") or 0)
        if champ not in (h,a):continue
        if champ==h:gf,ga=hs,aw; win=m.get("winner_player_id")==m.get("home_player_id")
        else:gf,ga=aw,hs; win=m.get("winner_player_id")==m.get("away_player_id")
        rec["gf"]+=gf;rec["ga"]+=ga
        if hs==aw and m.get("winner_player_id") is None:rec["d"]+=1
        elif win:rec["w"]+=1
        else:rec["l"]+=1
    return rec
...
def _mot_text(mot):
    ...
    if mot.get("home_penalties") is not None and mot.get("away_penalties") is not None:score+=f" (k. {mot['home_penalties']}:{mot['away_penalties']})"
    title=f"{mot.get('home','—')} {score} {mot.get('away','—')}"
    labels={"GROUP":"Grupa","LEAGUE":"Liga","QF":"Ćwierćfinał","BARRAGE":"Baraż","SF":"Półfinał","FINAL":"Finał","WB":"Winners","LB":"Losers","WB_FINAL":"Finał Winners","LB_FINAL":"Finał Losers"}
    detail=labels.get(str(mot.get("stage") or ""),str(mot.get("stage") or "—"))
    if mot.get("group_name"):detail+=f" • grupa {mot['group_name']}"
    return title,detail
...
def generate_summary_png(bundle:dict,summary:dict,format_labels:dict[str,str],official_no:int|None=None)->bytes:
    ...
    t=bundle.get("tournament",{});meta=bundle.get("meta",{})
    ...
    champ=str(summary.get("champion") or "—");runner=str(summary.get("runner_up") or "—")
    champ_team=_team_for(bundle,champ);runner_team=_team_for(bundle,runner)
    final=_final(bundle)
    final_score="—"
    if final:
        final_score=f"{final.get('home_score')}:{final.get('away_score')}"
        if final.get("home_penalties") is not None:final_score+=f"  k. {final.get('home_penalties')}:{final.get('away_penalties')}"

    # Main result
    _rr(draw,(58,250,1022,500),fill=PANEL,outline="#334563",r=32)
    draw.text((92,280),"MISTRZ",font=_font(29,True),fill=GOLD)
    draw.text((92,325),champ,font=_fit(draw,champ,590,72,40,True),fill=TEXT)
    draw.text((94,409),champ_team,font=_fit(draw,champ_team,580,30,22),fill=MUTED)
    draw.text((94,451),f"Finalista: {runner} • {runner_team}",font=_fit(draw,f"Finalista: {runner} • {runner_team}",600,26,18),fill="#d1d9e8")
    ...
    # Row 1
    _rr(draw,(58,530,515,715));_rr(draw,(545,530,1022,715))
    scorer=summary.get("real_top_scorer") or {}
    draw.text((86,558),"STRZELEC TURNIEJU",font=_font(25,True),fill=GREEN)
    scorer_name=str(scorer.get("name") or "—");goals=int(scorer.get("goals") or 0)
    ...
    # Row 2: champion record + tournament numbers
    _rr(draw,(58,745,515,938));_rr(draw,(545,745,1022,938))
    rec=_champion_record(bundle,champ)
    draw.text((86,774),"MISTRZ W TURNIEJU",font=_font(25,True),fill=PINK)
    draw.text((86,826),f"{rec['w']}W  •  {rec['d']}R  •  {rec['l']}P",font=_font(35,True),fill=TEXT)
    draw.text((86,879),f"Bramki {rec['gf']}:{rec['ga']}",font=_font(27),fill=MUTED)
    ...
    draw.text((573,826),f"{len(played)} meczów  •  {total_goals} goli",font=_font(31,True),fill=TEXT)
    draw.text((573,879),f"Średnio {avg:.1f} gola / mecz",font=_font(25),fill=MUTED)

    # Footer facts, larger and readable
    facts=[]
    if summary.get("biggest"):
        x=summary["biggest"];facts.append(f"Największe zwycięstwo: {x['home']} {x['score']} {x['away']}")
    if summary.get("highest"):
        x=summary["highest"];facts.append(f"Najwięcej goli w meczu: {x['home']} {x['score']} {x['away']}")
    y=965
    for fact in facts[:2]:
        draw.text((74,y),fact,font=_fit(draw,fact,930,21,17),fill="#cbd5e1");y+=30
