...
def gala_nav_visible(year:int|None=None) -> bool:
    try:return bool(db.awards_gala_nav_visible(int(year or datetime.now().year)))
...
def render_start(public_mode:bool=False):
    ...
    gala_visible=gala_nav_visible()
    if current=="gala" and not gala_visible:
        current="awards";st.session_state[state_key]=current
    subtitles={
        ...
        "gala":"FIFA Night Awards — telewizyjna gala laureatów.",
        "settings":"Sterowanie i ustawienia dostępu.",
    }
    hero(subtitles.get(current,subtitles["fifa"]))
    nav=[("fifa","🎮 FIFA NIGHT"),("stats","📊 STATYSTYKI"),("awards","🏆 AWARDS")]
    if gala_visible:nav.append(("gala","🎬 GALA"))
    nav.append(("settings","⚙️ USTAWIENIA"))
    ...
    if current=="gala":render_gala_mode(readonly=public_mode);return
...
def render_gala_mode(readonly: bool = False):
    year=datetime.now().year
    status=db.awards_gala_status(year)
    st.markdown("<style>.block-container{max-width:1500px!important;padding-top:.55rem!important}</style>",unsafe_allow_html=True)
    render_awards_gala_tv(status,fifa_night_api_url(),year)
    if status.get("test_mode") and controller_access() and not readonly:
        with st.expander("🧪 Sterowanie testowe gali",expanded=False):
            st.caption("Tylko na czas budowy modułu. Docelowo galą steruje wyłącznie telefon, a zakładka pojawi się dopiero po kompletnym wyborze laureatów.")
            st.caption(f"Gotowość: {int(status.get('selected_count') or 0)}/{int(status.get('required_count') or 18)} laureatów.")
            missing=status.get("missing_selection_details") or []
            unavailable=status.get("unavailable") or []
            if missing:st.warning("Brak wyboru: " + " • ".join(str(x.get('label') or x.get('key')) for x in missing))
            if unavailable:st.warning("Brak kandydatów — w próbie kategorie zostaną pominięte: " + " • ".join(str(x.get('label') or x.get('key')) for x in unavailable))
            c1,c2,c3=st.columns(3)
            with c1:
                if st.button("▶️ START / OD NOWA",use_container_width=True,key=f"gala_test_start_{year}"):
                    try:db.start_awards_gala(year);rr()
                    except ValueError as e:st.error(str(e))
            with c2:
                if st.button("⏭️ NASTĘPNA",use_container_width=True,key=f"gala_test_next_{year}"):
                    try:db.advance_awards_gala(year);rr()
                    except ValueError as e:st.error(str(e))
            with c3:
                if st.button("↻ POWTÓRZ",use_container_width=True,key=f"gala_test_replay_{year}",disabled=not bool(status.get("can_replay"))):
                    try:db.replay_awards_gala_category(year);rr()
                    except ValueError as e:st.error(str(e))
            if st.button("🧹 RESET GALI",use_container_width=True,key=f"gala_test_reset_{year}"):
                db.reset_awards_gala(year);rr()
    elif not status.get("available"):
        st.info("Gala pojawi się tutaj po wybraniu laureatów wszystkich kategorii.")
...
def render_viewer_live(t):
    ...
    gala_visible=gala_nav_visible()
    if not gala_visible and st.session_state.get("viewer_view")=="🎬 GALA":st.session_state["viewer_view"]="📺 LIVE"
    opts=["📺 LIVE","📅 Terminarz","📊 Statystyki","🏆 AWARDS"]
    if gala_visible:opts.append("🎬 GALA")
    opts.append("⚙️ Ustawienia")
    view=st.segmented_control("Widok",opts,default=opts[0],key="viewer_view",label_visibility="collapsed") or opts[0]
    if view=="📺 LIVE":render_tv_screen(t["id"])
    elif view=="📅 Terminarz":render_schedule(t)
    elif view=="📊 Statystyki":render_stats(t,readonly=True)
    elif view=="🏆 AWARDS":render_awards(readonly=True)
    elif view=="🎬 GALA":render_gala_mode(readonly=True)
...
def render_live(t,public_test:bool=False):
    ...
    gala_visible=gala_nav_visible()
    if not gala_visible and st.session_state.get("view")=="🎬 GALA":st.session_state["view"]="🏠 Ekran główny"
    opts=["🏠 Ekran główny","📺 TV","📅 Terminarz","📊 Statystyki","🏆 AWARDS"]
    if gala_visible:opts.append("🎬 GALA")
    opts.append("⚙️ Ustawienia")
    view=st.segmented_control("Widok",opts,default=opts[0],key="view",label_visibility="collapsed") or opts[0]
    if view=="🏠 Ekran główny":live(t["id"]);reset_controls(t,"live")
    elif view=="📺 TV":render_tv_screen(t["id"])
    elif view=="📅 Terminarz":render_schedule(t)
    elif view=="📊 Statystyki":render_stats(t,readonly=public_test)
    elif view=="🏆 AWARDS":render_awards(readonly=public_test)
    elif view=="🎬 GALA":render_gala_mode(readonly=public_test)
    else:render_access_settings()
