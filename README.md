...
- 1 vs 1 — 1 mecz.
- 3 graczy — liga każdy z każdym + finał (4).
- 4 graczy — liga + finał (7) albo Double Elimination (6).
- 5 graczy — Double Elimination (8) albo liga + finał (11).
- 6 graczy — 2×3 + SF + finał (9), 2×3 + QF + SF + finał (11), albo Double Elimination (10).
- 7 graczy — Double Elimination (12), grupy 4+3 + QF + SF + finał (14), albo grupy 4+3 + SF + finał (12).
- 8 graczy — grupy 4+4 + SF + finał (15), Double Elimination (14), grupy 4+4 + baraże + SF + finał (17), albo Swiss 3 rundy + TOP4 (15).
- 9 graczy — 3×3 + Final Four (12), 3×3 + baraże + Final Three (15), 3×3 + TOP8 (16), albo Double Elimination z play-inem (16). Final Three występuje wyłącznie w wariancie 9-osobowym.
- 10 graczy — 2×5 + SF + finał (23), Swiss 3 rundy + TOP4 (18), albo Double Elimination z dwoma play-inami (18).

Stałe crossy wynikające z formatu, np. `1A–2B` i `1B–2A`, pozostają deterministyczne. Animowane losowanie jest pokazywane tylko wtedy, gdy istnieją co najmniej dwa równorzędne legalne warianty.

## Wersja gry i drużyny

Przed turniejem wybierasz `EA SPORTS FC 26` albo `EA SPORTS FC 27`. Stara historia jest traktowana jako FC26.

### FC26
Normalna pula legacy: Bayern Monachium, FC Barcelona, PSG, Liverpool. Arsenal i Manchester City pozostają Wild Cardami.

### FC27
Normalna pula: PSG, Bayern Monachium, FC Barcelona, Arsenal, Manchester City. Przy 5 graczach nie ma WC; od 6 graczy każdy kolejny slot jest Wild Cardem.

Status Wild Card jest liczony per turniej według wersji gry, ale historia graczy, Awards, rekordy, kamienie milowe, H2H i finanse pozostają wspólne dla FIFA Night. Tylko Live Team Rating jest wersjonowany FC26/FC27.

Real Madryt może działać jako helper wyłącznie dla gracza `Gra za kasę = NIE`. Nie wchodzi do Live Team Ratingu, rankingu drużyn ani Drużyny Roku; statystyki gracza i przeciwnika z takiego meczu są liczone normalnie.
...
Historia pokazuje zakończone turnieje oraz 1 VS 1, a także oficjalne turnieje zakończone jako `NIEDOKOŃCZONY`. Mechanizm blokowania/odblokowywania Historii został usunięty. Z poziomu Historii można skasować cały mecz 1 VS 1 albo cały turniej; nie usuwa się pojedynczego meczu ze środka zamkniętego turnieju. Operacje kasowania wymagają autoryzacji administratora/sterowania.

Rozegrane mecze z turnieju `NIEDOKOŃCZONEGO` pozostają w oficjalnych statystykach, H2H, strzelcach i danych Awards, ale taki turniej nie ma mistrza, podium, tytułu ani rozliczenia.
...
Główne panele poza aktywną rozgrywką to tylko `🎮 FIFA NIGHT`, `📊 STATYSTYKI`, `🏆 AWARDS` i `⚙️ USTAWIENIA`. W panelu FIFA Night wybiera się od razu wariant `1 vs 1` albo liczbę graczy `3–10` — bez dodatkowego przełącznika trybu. Historia turniejów znajduje się w `Statystyki → Historia`. Drużyny i strzelcy są zebrani razem w `Statystyki → Drużyny i strzelcy`.

### Hotfix 29 — oficjalny FIFA Night zakończony przed końcem

Oficjalnego turnieju 3–10 graczy nie trzeba już ani dogrywać do końca, ani resetować, jeśli wieczór kończy się wcześniej. Na ekranie aktywnego oficjalnego turnieju organizator ma opcję `⏹️ Zakończ FIFA Night jako niedokończony`.
