...
Repozytorium zawiera natywną aplikację React Native + Expo w katalogu `mobile/`. Architektura to `Android APK -> FastAPI/Render -> Neon PostgreSQL <- Streamlit/TV`; aplikacja mobilna nie łączy się bezpośrednio z bazą i korzysta wyłącznie z `mobile_api.py`.

Kod Mobile 1.0 jest przygotowany jako źródło do finalnego builda APK. Lokalny pakiet regresyjny uruchamia się poleceniem `python tests/mobile_api_smoke.py`; aktualny release source przechodzi 49/49 testów backend/logiki. Instrukcja walidacji i builda jest w `MOBILE_1_0_RELEASE.md`, a szczegóły mobilki w `mobile/README.md`.
